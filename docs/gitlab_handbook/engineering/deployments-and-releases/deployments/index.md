...
are run in staging and production environments through the post-deploy migration (PDM) pipeline. The PDM is executed manually by release managers as part of standard operational procedures. To minimize the risk of production incidents while ensuring minimal organic traffic, release managers schedule PDM execution during the transition period between APAC end-of-business and EMEA start-of-business hours (between approximately 06:00 UTC and 09:00 UTC).

While the standard cadence targets daily execution, the actual PDM execution depends on GitLab.com availability and may be deferred to accommodate critical release management processes.
...
#### Blocking deployments with an incident

> **Warning:** When the `blocks deployments` flag is active, migrations (not post deployment migrations) will still continue to be executed. See [What does the `blocks deployments` flag do?](#what-does-the-blocks-deployments-flag-do) for details.

Anyone can **halt or block a deployment to Production** by:

1. [Declaring](https://gitlab.com/gitlab-com/runbooks/docs/incident-io-onboard/incident-management.md#how-to-raise-an-incident) an [incident](/handbook/engineering/infrastructure-platforms/incident-management/#reporting-an-incident):
...
##### What does the `blocks deployments` flag do?

When the `blocks deployments` flag is set:

- **Canary deployments continue**: Deployments to staging-canary (gstg-cny) and production-canary (gprd-cny) are not blocked
- **Migrations are not blocked**: Database migrations execute as part of canary deployments. If you would like to block migrations, please [contact Release Managers](/handbook/engineering/deployments-and-releases/#how-to-contact-a-release-manager)
- **Main environment promotions are blocked**: Promotions to staging (gstg) and production (gprd) are prevented
- **Post-deploy migrations are blocked**: The post-deployment migration pipeline is blocked

This design ensures continuous validation of code changes and migrations while creating deploy pressure only for main environment promotions.

#### Blocking deployments during change lock periods
...
1. An [active incident with the `blocks deployment` label](/handbook/engineering/infrastructure-platforms/incident-management/#labeling).
...
Release Managers may decide, with input from the [EOC](/handbook/engineering/infrastructure-platforms/incident-management/#incident-response-roles) to override a block and continue with the deployment.
...
In case of high-severity bugs, please proceed to [report an incident](/handbook/engineering/infrastructure-platforms/incident-management/#reporting-an-incident) to track
events and response.
...
### I found a problem with a post-deployment migration, what do I do next?

To prevent a post-deployment migration from being executed on GitLab.com, immediately follow the steps in [Deployment blockers](#deployment-blockers) and alert Release Managers. Work towards reverting the post-deployment migration to ensure it is not part of the monthly release.
...
| Release manager schedule | [Link](/handbook/engineering/releases/release-managers/) |
