...
Switchboard is a team within the [Dedicated Group](/handbook/engineering/infrastructure-platforms/gitlab-dedicated/). Our mission is to empower external GitLab Dedicated customers to manage their tenant environments and reduce the operational overhead on the Environment Automation team so we can scale up the GitLab Dedicated offering. We follow the same processes as listed on the [Dedicated Group](/handbook/engineering/infrastructure-platforms/gitlab-dedicated/), unless a difference exists which is explicitly noted on this page.

### Resources

- [Switchboard handbook page](/handbook/engineering/infrastructure-platforms/gitlab-dedicated/switchboard/)
...
- In case you want to get attention please use the specific team handle, `@gitlab-dedicated/switchboard`, as defined in [Dedicated group hierarchy](/handbook/engineering/infrastructure-platforms/gitlab-dedicated/#gitlab-group-hierarchy)
...
- Create an [Access Request](https://gitlab.com/gitlab-com/team-member-epics/access-requests/-/issues/new?issuable_template=Individual_Bulk_Access_Request) specifying
  ...
  - Role required (see [Role description below](#roles))
...
<details><summary>Switchboard internal Roles & Permissions</summary>
...
- **Migration Operator**: Temporarily operate the tenant instance in preparation for migration. This role is restricted to Site Reliability Engineers (SREs) and Switchboard team members.
- **Hosted Runner Operator**: View tenant instance & manage hosted runner config. This role is restricted to Site Reliability Engineers (SREs) and Switchboard team members.
...
DRIs should also drive a [pivot to synchronous discussion](/handbook/company/culture/all-remote/asynchronous/#gitlab-experts-advise-on-when-to-use-sync-vs-async) when a thread is not yielding a timely conclusion.
...
the [Dedicated group principles](/handbook/engineering/infrastructure-platforms/gitlab-dedicated/#merge-requests) when requesting merge request reviews.
...
Feature flags in Switchboard enable safe, incremental rollouts of new functionality and provide kill switches for rapid rollback when issues arise.

#### Definition of Done

**Feature flag code must be removed after successful rollout.** A feature is not considered complete until:

- The feature has been rolled out to 100% of applicable tenants
- The feature has been stable for a previously agreed upon amount of time (usually determined at epic planning level), with a minimum of 1 week.
- All feature flag conditionals have been removed from the codebase

#### Working with Feature Flags

- **Assign an owner** - Every feature flag must have a designated owner(s) responsible for its removal. The owner(s) will be set as the assigned individual(s) in the removal issue.
- **Create cleanup issue upfront** - When introducing a feature flag, immediately create a follow-up issue for flag removal and link it to the implementation issue
- **Document removal plan in epic** - The epic introducing the feature flag must include the expected removal timeline and criteria for cleanup in the feature flag section
- **Default to short-lived** - Flags should typically be removed within 1-2 weeks of 100% rollout

Stale feature flags create technical debt and obscure the intended behaviour of our system. Timely cleanup is essential to maintaining code quality.
