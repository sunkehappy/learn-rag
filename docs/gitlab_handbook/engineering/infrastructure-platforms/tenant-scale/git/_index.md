...
[Responsibilities](/job-description-library/engineering/backend-engineer/#git) |
...
- Respond to feedback that you have received soon, ideally within two or three
  working days. A fast exchange helps with having a fruitful discussion and
...
## Workflow in downstream GitLab

### Upstream first

The Git team has an upstream-first approach: functionality that we want to use
at GitLab first needs to be merged upstream before we can make use of it. This
ensures that we do not eventually end up with a fork of Git that has diverged
from upstream Git. Such a fork would incur additional maintenance cost and has
increased risk of breakage due to a significantly smaller test user base.

Backports are the exception to this rule. Backports are changes that are merged
into a version that's not deployed yet. That can either be an unreleased version
(i.e. changes merged into `master`) or when running a version that's older than
the latest release. Backports can happen when:

- We face issues that in production require bug or performance fixes.
- Security fixes are made that are relevant to GitLab.

To perform such backports we use GitLab-specific branches in our
[mirror of the Git project](https://gitlab.com/gitlab-org/git). Each release may
have a corresponding GitLab-specific branch, e.g. `gitlab-git-v2.50`. This
branch initially points to the latest maintenance release of the corresponding
release track, e.g. v2.50.1.

In case we need to backport fixes, the individual contributor is expected to
create a merge request that targets the GitLab-specific branch. In the source
branch they cherry-pick the patches they want to include, and this merge request
undergoes normal review and will eventually be merged.

Once merged, we create a separate tag. E.g. if the latest upstream release on
that branch is Git v2.50.1, we would create a new tag `v2.50.1.gl1`. If
additional patches need to be backported at a later point in time we would
create `v2.50.1.gl2`.

If the upstream project eventually tags `v2.50.2`, then those changes need to be
merged into the GitLab-specific branch and a new `v2.50.2.gl1` tag is created.
Note that this is only required if this new upstream version does not contain
the GitLab-specific patches.

### Source of Truth

Gitaly is the source of truth for which Git version is used on our production
...
contains the build instructions for Git as well as the specific version that
Gitaly should build.

The update process is currently manual, where the Git team is responsible for
bumping the version on new releases. We are currently in the process of
[changing this](https://gitlab.com/groups/gitlab-org/-/epics/17694) so that we
do not use tagged releases anymore. Instead, we will start deploying Git from
the `master` branch with a lag of about two weeks. The process will be automated
via Renovate.
