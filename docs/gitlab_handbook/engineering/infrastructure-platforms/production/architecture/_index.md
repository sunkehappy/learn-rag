...
- [CI Service Architecture](ci-architecture.md)
- [dev.gitlab.org Architecture](supporting-architecture.md#dev-gitlab-org)
- [ops.gitlab.net Architecture](supporting-architecture.md#ops-gitlab-net)
- [version.gitlab.com Architecture](supporting-architecture.md)
...
Alerting for the cluster uses generated [rules](https://gitlab.com/gitlab-com/runbooks/-/tree/master/mimir-rules) that feed up to our [overall SLA](/handbook/engineering/monitoring/) for the platform.

Logging is configured using [fluentd-elasticsearch](https://gitlab.com/gitlab-com/gl-infra/argocd/apps/-/blob/main/services/fluentd-elasticsearch/README.md) where the logs for every pod is forwarded to a unique Elasticsearch index. fluentd-elasticsearch is deployed in the namespace `logging`.
...
For more information about secret management see the runbook for [Chef secrets using GKMS](https://gitlab.com/gitlab-com/runbooks/-/blob/master/docs/fleet-management/config_management/gkms-chef-secrets.md) and [how we manage secrets in Kubernetes](https://gitlab.com/gitlab-com/gl-infra/k8s-workloads/gitlab-com#gitlab-secrets).
