...
- Ask for opinions in [#infrastructure_platforms](https://gitlab.enterprise.slack.com/archives/C02D1HQRTKQ) or [#s_production_engineering](https://gitlab.enterprise.slack.com/archives/C07U6SAKS4D)
...
We do not create Hard PCLs for reasons other than low team-member coverage or periods of significant instability of our platforms. If you are concerned about platform stability during an event, please look at the Soft PCL option.
...
- Infrastructure changes with criticality level C2 or higher
- Post-deployment migrations (except in emergencies with EOC approval)
- Feature flag toggles that require a change management issue (follow the change management process)

**What is ALLOWED:**

- Infrastructure changes with criticality level C3 or C4
- Code deployments to canary (we have tools to control canary impact)
- Code deployments to production without post-deployment migrations (requires EOC coordination)
- Feature flag toggles that don't require a change management issue
- Emergency changes for C1 and C2 infrastructure (requires EOC and Incident Manager On Call interaction)

For feature flag guidelines during Soft PCL, see [Feature Flags and the Change Management Process](/handbook/engineering/infrastructure-platforms/change-management#feature-flags-and-the-change-management-process).

### Hard PCL

Hard PCLs include all restrictions from Soft PCL, plus additional restrictions on code deployments and all infrastructure changes regardless of criticality level.

**What is BLOCKED:**

- All code deployments (to canary and production)
- All infrastructure changes (C1, C2, C3, C4)
- All feature flag toggles
- Post-deployment migrations

**What is ALLOWED:**

- Changes required for active S1/S2 incident response (requires approval from both EOC and Incident Manager On Call)

**Approval process for S1/S2 incident changes:**

In case of an active S1/S2 incident, the EOC must interact with the Incident Manager On Call prior to making any decision. It is at their discretion to approve whether a change should be executed. If approved, the Incident Manager On Call must inform the [Infrastructure Leadership Escalation](/handbook/engineering/infrastructure-platforms/incident-management/#infrastructure-leadership-responsibilities) of this decision (who will inform the executive team as necessary).

### Requesting exceptions

If you need something deployed during a PCL:

1. Create an issue in the [Production issue tracker](https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/issues/new) that includes clear justification for why this change should be exempt
2. Reference your work and project in this issue, and link the new issue to the relevant PCL issue. Include the estimated nARR impact
3. Ensure that the new issue has approval from the VP Infrastructure Platforms (or their delegate) documented in a comment
4. Coordinate with the EOC and release managers to have this change shipped

### Declaring a PCL

Production Change Locks (PCLs) can be either **planned** (for recurring events like holidays) or **emergency** (for unexpected situations requiring immediate action). For recurring PCLs like Thanksgiving and end-of-year holidays, see the [Recent PCLs table](/handbook/engineering/infrastructure-platforms/change-management/#recent-pcls) which documents the standard dates.
...
For recurring PCLs (such as Thanksgiving or end-of-year holidays), follow these steps:

1. **Create tracking issues and entries** (Production Engineering)
   - Create a [C1 Change issue](https://gitlab.com/gitlab-com/gl-infra/production/-/issues/new?issuable_template=change_pcl&issue[title]=Production+Change+Lock) that will block deployments and feature flags
   - Create a corresponding entry in [gl-infra/change-lock](https://gitlab.com/gitlab-com/gl-infra/change-lock)
   - If needed, add specific dates to the [Recent PCLs table](/handbook/engineering/infrastructure-platforms/change-management/#recent-pcls) in this handbook page (recurring entries are already documented)
   - Mark the issue as `~change::in-progress` at the start of the PCL period

2. **Obtain review and approval** (Software Delivery)
   - Add Software Delivery Engineering Managers (EMs) as reviewers on the change-lock merge request
   - Software Delivery EMs review and approve the changes

3. **Communicate to stakeholders** (Software Delivery)
   - Notify the Engineering Organization through the `#engineering-fyi` Slack channel
   - Add the entry to the [Engineering Week In Review](https://docs.google.com/document/d/1JBdCl3MAOSdlgq3kzzRmtzTsFWsTIQ9iQg0RHhMht6E/edit#heading=h.wl5oryd6kv3u)
   - Use [ChatOps Notify](https://gitlab.com/gitlab-org/release/docs/-/tree/master/release_manager?ref_type=heads#utilities) to communicate the PCL to relevant Slack channels
   - Provide details on any changes required for deployment and release processes

4. **Clean up after PCL ends** (Production Engineering)
   - Close the C1 Change issue
   - Remove any specific dated entries from the Recent PCLs table (keep recurring entries)

#### Declaring an emergency PCL

For unplanned PCLs (such as during critical incidents or unexpected events):

1. **Assess the situation**
   - Check with `@release-managers` in the `#releases` Slack channel to confirm the state of the monthly release or any patch releases
   - Check with security in the `#security` Slack channel if there are any imminent security patches that release managers may not be notified of yet
   - Check with `@incident-managers` and `@sre-oncall` in the Slack `#production` channel to see if there are any concerns
   - Inform the [Infrastructure Leadership Escalation](/handbook/engineering/infrastructure-platforms/incident-management/#infrastructure-leadership-responsibilities)

2. **Follow the planned PCL process with expedited approvals**
   - Once the need is confirmed, follow steps 1-3 from the planned PCL process above
   - Expedite approvals and communications given the urgent nature
   - Clearly document the reason for the emergency PCL in the C1 Change issue
