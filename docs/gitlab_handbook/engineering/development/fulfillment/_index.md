...
In addition to the [Fulfillment Product Direction](/handbook/product/groups/fulfillment/direction/fulfillment_section/), the Fulfillment Development Sub-department strives to:
...
- We follow the [Product Designer workflows](/handbook/product/ux/product-designer/) and [UX Researcher workflows](/handbook/product/ux/experience-research/) described in the [Product Design section](/handbook/product/ux/) of the handbook.
...
- We use labels to track our issues:
  ...
  - `design weight::` for UX issue weights
...
- Issue weights should follow the label definitions and are applied using the ~'design weight:" label.
...
**Tier 2 SME On-Call Escalation**

Fulfillment has implemented a [Tier 2 SME on-call rotation](https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/issues/27741) for incident response. This process provides structured escalation to subject matter experts within the Fulfillment team.

**When Tier 2 gets paged:**

- During incidents or outages, the Engineer on Call (EOC) can escalate to the Fulfillment Tier 2 SME on-call rotation through incident.io
- The Tier 2 SME rotation follows a [defined escalation path](/handbook/engineering/infrastructure-platforms/incident-management/on-call/tier-2/):
  - **Level 1**: Current SME on-call based on schedule rotation (EMEA, AMER, or APAC)
  - **Level 2**: If Level 1 is not acknowledged within 15 minutes, escalation proceeds via round-robin to all team members
  - **Level 3**: Further escalation to [James Lopez](https://gitlab.com/jameslopez)

**Additional automated notifications for outages:**

- When an outage occurs, `#s_fulfillment_status` on Slack is notified and [James Lopez](https://gitlab.com/jameslopez) and [Vitaly Slobodin](https://gitlab.com/vitallium) receive automated phone pages

**General incident reporting:**

1. When an outage occurs, the [SRE on-call](/handbook/engineering/on-call/) is notified automatically. Incidents can also be [reported manually](/handbook/engineering/infrastructure-platforms/incident-management/#reporting-an-incident).
...
[Raising an incident](/handbook/engineering/infrastructure-platforms/incident-management/#report-an-incident-via-slack) is always an option if you need immediate attention on a blocking problem.
...
After the `8th`, the Fulfillment team conducts an [asynchronous retrospective](/handbook/engineering/careers/management/group-retrospectives/). You can find current and past retrospectives for Fulfillment in [https://gitlab.com/gl-retrospectives/fulfillment/issues/](https://gitlab.com/gl-retrospectives/fulfillment/issues/).
...
Review [these rules](/handbook/engineering/careers/management/group-retrospectives/) for an effective retrospective.
...
| [Fulfillment Weekly Updates](https://gitlab.com/gitlab-org/fulfillment-meta/-/issues?sort=created_date&state=all&label_name[]=weekly+update)                     | Weekly       | Async | Fulfillment PMs, EMs, UXMs                                |
...
| [Fulfillment PM / EM / UXM Syncs](https://docs.google.com/document/d/1UBfHiGK6BgJY76sFJ1qqlsRbtYhbBZETpaA4ftX_BBc/edit)                                    | Weekly       | Sync  | Fulfillment PMs, EMs, UXMs                                |
| OKR Check-Ins                                                                                            | Weekly       | Async | Fulfillment PMs, EMs, UXMs                                |
...
| Roadmap Planning                                                                                      | Monthly      | Sync  | Fulfillment PMs, EMs, UXMs                                |
...
| [Direction Review](/handbook/product/groups/fulfillment/direction/fulfillment_section/)                                                                                              | Quarterly    | Async | Fulfillment PMs, Product Leadership                             |
| OKR Planning                                                                                             | Quarterly    | Async | Fulfillment PMs, EMs, UXMs, Staff Engineers               |
...
1. For [internal support requests](/handbook/support/internal-support/#internal-requests) regarding licensing, subscriptions, trials, or any requests involving a customer, please fill out the [internal request form](https://support-super-form-gitlab-com-support-support-op-651f22e90ce6d7.gitlab.io/).
...
- [Product Vision](/handbook/product/groups/fulfillment/direction/fulfillment_section/)
