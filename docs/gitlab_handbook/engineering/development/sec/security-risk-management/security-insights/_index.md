...
We should aim to have work item **descriptions** remain the source of truth for all decisions relating to a task and/or any status update.

To assess if a description has sufficient details, consider the following prompt:

> I am a new engineer on a 2-month-long project, the deadline is in a week and I need to start working ASAP - what do I need to know?
...
Milestone tracking is strictly for tracking _when_ something will be released, and should not be mixed up or confused with `Time Tracking` or capacity, i.e; if an issue has a delivery milestone of `n+2`, we should not assume that equates to \~2 months of work!

#### Mutual Responsibility

Every team member has the **responsibility** to suggest improvements to issue hygiene. If you find yourself a bit confused or lacking some context while reading an issue/epic, please raise a flag so that the process can be improved 🙂
