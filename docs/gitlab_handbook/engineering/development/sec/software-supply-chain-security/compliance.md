...
1. If a feature flag is used, a [feature flag rollout plan](/handbook/engineering/workflow/development-processes/rollout-plans/) issue should be created and added to the appropriate epic. Support (`#support_gitlab-com`) will also be [notified](https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/issue_templates/Feature%20Flag%20Roll%20Out.md?plain=1#L94) if necessary.
...
In cases where verification in staging or production is unfeasible, the [staging-ref environment](/handbook/engineering/infrastructure-platforms/environments/staging-ref/) may be used. For complex setups, the DRI for the MR should work with a domain expert to ensure verification steps are clear and correct.
