...
For assistance with creating detailed implementation plans during the refinement process, see our [Issue Refinement AI prompt](/handbook/engineering/development/sec/software-supply-chain-security/pipeline-security/ai-prompts/#issue-refinement).

#### Implementation Template

During the refinement process (Phase 2), issues must use the [GitLab Implementation issue template](https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/issue_templates/Implementation.md) to ensure consistency and clarity. This template emphasizes:

- **Why are we doing this work**: Clear articulation of the problem being solved, including both the "what" and "why"
- **Non-functional requirements**: If required,  details around items such as documentation, feature flags, performance, testing, etc.
- **Implementation Plan**: Detailed technical approach and implementation steps
- **Verification Steps**: Step-by-step instructions for verifying the implementation

When refining issues, engineers should:

1. Use the Implementation template structure in the issue description
2. Ensure all sections are completed with relevant information
3. Provide enough detail that any engineer can understand the approach and start development immediately
4. Include all necessary information in the issue description (not in comments) as the single source of truth.  Comments should be used for additional context as needed.

This standardized approach improves clarity, reduces refinement time, and ensures all team members have consistent expectations for implementation issues.
...
- If the issue will be worked on by Software Supply Chain Security:Pipeline Security engineers, it has a [weight](/handbook/engineering/devops/create/source-code/backend/#weighting-issues)
...
When an engineer is actively working on an issue (workflow of ~workflow::"In dev" or further right on the current milestone) or has significant details to share on an assigned issue, they will leave status updates as top-level comments in the issue to provide an end-of-week summary.

The status comment should include:

- Updated health status
- Any blockers
- Notes on what was accomplished
- Whether review has started
- Any other information the engineer feels is beneficial
- cc the EM on the update provided

If multiple people are working on the issue, also include whether this is a frontend or backend update. An update for each MR associated with the issue should be included in the update comment. Engineers should also update the [health status](https://docs.gitlab.com/ee/user/project/issues/#health-status) of the issue at this time.
...
We use [geekbot](https://geekbot.com/) integrated with Slack for our async standups. The purpose of the standup is to keep the team informed about what everyone is working on, and to surface blockers so we can eliminate them. The standup bot runs on Tuesday and Friday at 10am in the team members local time and will ask 3 questions:

1. What have you done since last update?
2. What are you currently working on?
3. Anything blocking your progress?

On a weekly basis, team members share a brief update for any issues assigned to the current milestone.
Updates are shared in issues in the form of comments following the [issue progress updates](#issue-progress-updates) guidelines.
They should be posted on Friday (or on the last working day of the week) so that the engineering manager
can publish a weekly update for the group on the following Monday.
...
For assistance with analyzing retrospective comments and extracting actionable insights, see our [Retrospective Summary AI prompt](/handbook/engineering/development/sec/software-supply-chain-security/pipeline-security/ai-prompts/#retrospective-summary).
...
We maintain a collection of AI prompts designed for use with GitLab Duo to assist with common day-to-day tasks. These prompts are tailored to our team's specific workflows and processes.
