...
- [Wider Community Merge Request Triage](/handbook/engineering/infrastructure-platforms/developer-experience/merge-request-triage/)
- [Retrospectives](/handbook/engineering/careers/management/group-retrospectives/)
...
Refer to the [SLA exceptions handbook page](/handbook/security/product-security/vulnerability-management/sla-exceptions.md) for process details.
...
For each item, investigate and either [dismiss](#dismissing-a-vulnerability) or [confirm](#confirming-a-vulnerability) it. If it's not clear whether there's indeed a threat, [escalate](#creating-security-issues) to our [Application Security team](/handbook/security/product-security/security-platforms-architecture/application-security/).
...
Developers reporting the security issue should help the [Application Security team](/handbook/security/product-security/security-platforms-architecture/application-security/) assess the impact of the vulnerability, and update the issue description with an `Impact` section.
...
- [rhsa2ovaloracle](https://gitlab.com/gitlab-org/security-products/rhsa2ovaloracle): generates Oracle advisories used by the analyzer.
...
To test FIPS images, we host our own runner instances.
You can find these by filtering the [VM instances](https://console.cloud.google.com/compute/overview?invt=Ab6F3g&project=group-secure-a89fe7).
for `sca-fips-runner-`. If you need to provision a new runner, use the
[provisioning script](https://gitlab.com/gitlab-org/security-products/analyzers/gemnasium/-/blob/6f1125442490f931a61d30f2e24bb31e473a3927/scripts/create_fips_runner.sh)
and follow the included directions.
...
- [Dependency Scanning SBOM scan API](https://log.gprd.gitlab.net/app/r/s/fhYY5)
- [Dependency Scanning analyzers metrics](https://10az.online.tableau.com/#/site/gitlab/workbooks/3648287/views)
- [Multi Container Scanning metrics](https://10az.online.tableau.com/#/site/gitlab/views/PDSecMetricHub/StandardView?:iid=1)

## Related resources

- [List of controllers, API endpoints, Sidekiq workers and Database tables for CA team](https://gitlab-com.gitlab.io/gl-infra/platform/stage-groups-index/composition-analysis.html)
...
The following table presents the various GitLab projects that can generate slack alerts.
Please notice that this doesn't include alerts generated by Sentry.
If you change the name of any of the Slack channels, please update the integrations in the projects,
so that the alerts continue to function.

| Project            | Target slack channel                                                                         | Additional Info                                                                                                          |
|--------------------|------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| [Gemnasium](https://gitlab.com/gitlab-org/security-products/analyzers/gemnasium)          | `#g_ast-composition-analysis-alerts`   | Failures on default branch and new releases                                                                                                                         |
...
| [Container Scanning](https://gitlab.com/gitlab-org/security-products/analyzers/container-scanning) | `#f_container_scanning` | Failures on default branch and new releases                                                                                               |
...
| [rhsa2ovaloracle](https://gitlab.com/gitlab-org/security-products/rhsa2ovaloracle)      | `#g_ast-composition-analysis-alerts`  | Failures on default branch |
| [Dependency Scanning Analyzer](https://gitlab.com/gitlab-org/security-products/analyzers/dependency-scanning)  | `#g_ast-composition-analysis-alerts`  | Failures on default branch and new releases |
