...
Secret Detection(SD) analyzers use [GitLab-managed ruleset](https://gitlab.com/gitlab-org/security-products/secret-detection/secret-detection-rules) when running Secret Detection scans. To increase the rule coverage of our analyzers, we collaborate with [Vulnerability Research](../vulnerability-research/_index.md) team members to develop rules supporting secret credential types of various vendors (companies). Though our analyzers already [support](https://docs.gitlab.com/user/application_security/secret_detection/detected_secrets/) 200+ popular credential types, we aim to cover more to expand rule coverage.

#### Expanding Rule Coverage

There are countless secret credential types exist across different companies that we could potentially support. We've been adding new rules based on customer demand, but to scale our ruleset coverage more systematically, we're planning to introduce a rotation process where each team member from Secret Detection group develop and release new rules on a regular basis.
