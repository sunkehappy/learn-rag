...
- [GitLab.com (infra) Architecture](/handbook/engineering/infrastructure-platforms/production/architecture/)
...
1. <a name="backend-processes"></a> [**Backend processes**](#tb-backend-processes)
    ...
    1. <a name="haproxy2nginx"></a> [HAProxy to NGINX](#tb-haproxy-ssl) (not measured)
       ...
       In this case, since we are tracking a web request, it would be the NGINX box in the
         "Web" box in the [production-architecture diagram](/handbook/engineering/infrastructure-platforms/production/architecture/); but alternatively the request can come in via API or a git command
