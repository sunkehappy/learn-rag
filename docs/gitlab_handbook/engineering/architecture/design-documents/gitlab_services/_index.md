...
The current work in [Analyze:Observability](/handbook/product/categories/#platform-insights-group) could be integrated into GitLab if it supports services.
...
- We don't want to tackle GitLab observability solution built by [Analyze:Observability](/handbook/product/categories/#platform-insights-group). The proposal here should treat it as one observability integration backend.
