...
> [!warning] No longer planned, subsumed into new blueprint
>
> This blueprint has been modified and folded into the [Migrate to ActiveRecord Encryption](/handbook/engineering/architecture/design-documents/migrate_to_activerecord_encryption/) blueprint. ("Rejected" is just the closest status we have available; "extended with some modifications" would be closer.) Please see that blueprint for our current plans.
...
4. [In Protocells, all cells will use the same secrets](https://gitlab.com/groups/gitlab-org/-/epics/13166#proposal),
