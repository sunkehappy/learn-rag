---
title: 'Protocells Disaster Recovery'
stage: core platform
group: Tenant Scale
status: proposed
toc_hide: true
...
Protocells is the first iteration of cells where Cells be operated independently of the Legacy Cell.
The target is to use Geo for Disaster Recovery which is the current DR strategy for GitLab Dedicated.
After Protocells, we intend to leverage [Next Gen Scalable Backup and Restore] for restoring data from backups.
...
| Protocells (without the primary cell) | _unknown_ | _unknown_ |
...
| Gitaly Cluster          | Gitaly Cluster consists of a single SPOF (single point of failure) node and remains so for Protocells. It requires a restore from backup in the case of a zonal failure. | <=30 min | <=1 hr for snapshot restore until WAL is available for restore. [^blueprint-dr] |
...
| Gitaly Cluster          | Initially, Gitaly Cluster consists of a single SPOF node and remains so for Protocells. It requires a rebuild in the case of a regional failure. | _Unknown_ | <=1 hr for snapshot restore until WAL is available for restore. [^blueprint-dr] |
