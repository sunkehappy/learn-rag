...
Subdomains (for example, `mycorp.gitlab.com` or `mygitlab.mycorp.com`) are not used for routing
because GitLab.com should feel like a single application regardless of how many Organizations a
user interacts with.
A subdomain per Organization or Cell creates a perception of strong isolation
between instances, which conflicts with the long-term goal of sharing users and data across
Organizations.
Using subdomains also introduces breaking changes (e.g. links, registry) for customers.
