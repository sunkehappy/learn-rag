...
title: "Cells ADR 014: No clusterwide syncing for Protocells"
toc_hide: true
---
...
1. There will be no application layer clusterwide synchronization in Protocells.
...
1. Organization / Cell table. Tables which may be better categorized as
   `gitlab_main_cell`.
1. User table. Tables related to users, and can be synchronized when there
   is a need to have cross-organization access.
...
As Users can only exist on one cell in Protocells, there is no need to
synchronize any user related data for now.
