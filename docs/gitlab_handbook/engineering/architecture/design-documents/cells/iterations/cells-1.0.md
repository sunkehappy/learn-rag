...
**NOTE:** The phase names Cells 1.0, 1.5, and 2.0 are replaced by Protocells.**
A lot of work done in Cells 1.0 carries over to Protocells as is.
...
1. Database properties:
    ...
    1. We require each table to be classified: to be Organization, or Cell-local.
    1. We follow a model of eventual consistency:
        1. All tables are stored in a Cell-local database.
            1. All cluster-wide attributes retain unique constraints across the whole cluster.
        1. The cluster-wide attribute are restricted to be modified by the Cell that is authoritative over the particular record:
...
1. What about data migration between Cells?
    ...
    would be migrated. For example, the current project import/export does neither migrate CI traces, nor job artifacts.
...
1. How would we synchronize `users` across Cells?
    ...
    We build out-of-band replication. We have yet to define
