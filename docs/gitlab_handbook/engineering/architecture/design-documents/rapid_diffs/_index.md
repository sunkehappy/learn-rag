...
1. The Time to First Diff (`mr-diffs-mark-first-diff-file-shown`) happens before 1.5 seconds mark for diffs of 1000 lines changed or less. 2 seconds above 1000 changed lines.
1. The metric for Fully Loaded (all diffs rendered) registers 25-40% improvement for diffs up to 20 files and 700 lines changed. See the [improvement formula](#fully-loaded-improvement-formula) for details.
...
The expected progression of the improvements based on the diff lines can be calculated using:
...
- `N` = line numbers of the diff
- `Improvement(N)` is expressed as a percentage
