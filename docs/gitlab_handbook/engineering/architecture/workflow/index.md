...
For internal design documents and additional resources, see the [internal design documents](https://internal.gitlab.com/handbook/engineering/architecture/).
...
1. For security-sensitive components, include a preliminary [threat model](/handbook/security/product-security/security-platforms-architecture/application-security/threat-modeling/) in your design document. This should identify key security boundaries, potential attack vectors, security assumptions, and mitigation strategies. Consult existing [threat models examples](https://gitlab.com/gitlab-com/gl-security/product-security/appsec/threat-models/-/tree/master) for guidance.
1. Work with the stakeholders and [domain experts](#domain-experts) to get your
   design reviewed and commented on. Design docs don't need to be comprehensive
...
1. [Application Security](/handbook/security/product-security/security-platforms-architecture/application-security/#contacting-us) can provide guidance and review for threat models when security-sensitive components are involved.
...
can note that it has been superseded and create a new ADR with the new
...
on the design phase, before you start writing production code. It is
...
- DRI should be identified in the epic as assignee and design document as author.
- DRI determines who needs to be consulted based on the scope and impact.

### Consultation vs. Blocking Authority

- **Consultation**: Groups that should be consulted but cannot block (for example, Domain Experts, Functional Experts).
- **Blocking Authority**: Groups that can block proposals (for example, App Sec for security-related changes).
- The distinction should be clear in the design document.
...
- Add all stakeholders as reviewers on the MR to use GitLab todos and review system as the main communication channel.
- Post in relevant Slack channels for additional visibility.
