...
- Backport MR authors are responsible for ensuring sufficient test coverages in every fix. If a test scenario cannot be automated within the same MR, the author is responsible for performing manual verification once the fix is merged and deployed to respective Release environments. Use the [Test Governance RFH](/handbook/engineering/infrastructure-platforms/developer-experience/test-governance/#request-for-help-process) process if you need help designing tests.
...
1. Engineer conduct robust testing to ensure that the backported code or backport package maintains full functionality within the system.
