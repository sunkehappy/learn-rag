...
If an incident becomes a blocker for MRs and deployments before being auto-escalated, the team member being impacted should refer to the [broken `master` escalation](#broken-master-escalation) steps to request help from the current [engineer on-call](/handbook/engineering/infrastructure-platforms/incident-management/#who-is-the-current-eoc) as early as needed.
...
1. If there is not a clear DRI or action to resolve, use the [dev escalation](/handbook/engineering/workflow/development-processes/infra-dev-escalation/process/) process to solicit help in the broken `master` incident.

**Note:** Stable branch failures follow the same process as described here, but incidents are tracked in [gitlab-org/release/tasks](https://gitlab.com/gitlab-org/release/tasks). See [stable branches documentation](/handbook/engineering/releases/stable_branches/#broken-stable-branches) for details.
...
1. Identification
   ...
   - If you identified that `master` fails **for a flaky reason**, and it cannot be reliably reproduced (i.e. running the failing spec locally or retrying the failing job):
      - [Fast Quarantine](https://gitlab.com/gitlab-org/quality/engineering-productivity/fast-quarantine/) the failing test to restore pipeline stability if the flakiness is continuously causing master pipeline incidents.
...
1. Backport the broken `master` incident to the [maintained stable branches](https://docs.gitlab.com/policy/maintenance/#maintained-versions). See [stable branches](#stable-branches)
...
broken at times. When this happens, check [What to do when the validation pipeline failed](/handbook/finance/jihu-support/jihu-validation-pipelines.html#what-to-do-when-the-validation-pipeline-failed) for more details.
...
It is the merge request author's responsibility to backport the following to the [maintained stable branches](https://docs.gitlab.com/policy/maintenance/#maintained-versions):

- Backport any master broken incident that impacts the [maintained versions](https://docs.gitlab.com/policy/maintenance/#maintained-versions).
...
Follow the [engineering runbook](https://gitlab.com/gitlab-org/release/docs/-/blob/master/general/patch/engineers.md#backporting-a-bug-fix-in-the-gitlab-project) to backport changes to stable branches.
...
For larger issues or issues that contain many different moving parts, you'll be likely working in a team. This team will typically consist of a [backend engineer](/job-description-library/engineering/backend-engineer/), a [frontend engineer](/job-description-library/engineering/development/frontend/), a [Product Designer](/job-description-library/product/product-designer/) and a [product manager](/job-description-library/product/product-manager/).
...
It's every [developers' responsibilities](/job-description-library/engineering/backend-engineer/#responsibilities) to triage and review code contributed by the rest of the community, and work with them to get it ready for production.
...
Product Managers are responsible for scheduling all issues in their respective [product areas](/handbook/product/categories/#devops-stages), including features, bugs, and tech debt. Product managers alone determine the [prioritization](/handbook/product/product-processes/cross-functional-prioritization/), but others are encouraged to influence the PMs decisions. The UX Lead and Engineering Leads are responsible for allocating people making sure things are done on time. Product Managers are *not* responsible for these activities, they are not project managers.
...
- **Monday, 19 days before the milestone begins**:
  ...
  - Development engineering manager provides product manager with prioritization input for `~type::maintenance` issues per [cross-functional prioritization](/handbook/engineering/workflow/cross-functional-prioritization/)
  - [Quality](/handbook/engineering/workflow/cross-functional-prioritization/) provides product manager with prioritization input for `~type::bug` issues per [cross-functional prioritization](/handbook/engineering/workflow/cross-functional-prioritization/)
...
- **Monday, 9 days after the milestone begins**:
  - The development lead for each stage/section coordinates a stage/section level review with the quad [cross-functional dashboard review process](/handbook/product/product-processes/cross-functional-prioritization/#cross-functional-dashboard-reviews). After the stages/section level reviews are complete, the VP of Development coordinates a summary review with the CTO, VP of Product, and VP of UX.
- **Wednesday, 11 days after the milestone begins**:
  - GitLab Bot opens [Group Retrospective](/handbook/engineering/careers/management/group-retrospectives/) issue for the current milestone.
- **Friday, the day the milestone ends**:
  ...
  - Merging by the milestone code cut-off (Friday) **does not guarantee** that the feature will be in the release. See [release timelines](/handbook/engineering/releases/monthly-releases#timelines).
  - Individual [release post entries](https://docs.gitlab.com/development/documentation/release_notes/) merged for all relevant issues.
  - By end of the day, milestone `%x.y` is expired.
- **On or around Wednesday, the day before the release date**:
  - [Group Retrospective issues](/handbook/engineering/careers/management/group-retrospectives/) are updated with shipped and missed deliverables and team-members are tagged in the discussion.
...
Refer to [release post content reviews](https://docs.gitlab.com/development/documentation/release_notes/) for additional deadlines.
...
1. assigned a [backlog label](https://gitlab.com/groups/gitlab-org/-/epics/18639), `~"backlog::prospective"` if it is expected to be worked on in the next 18 months, `~"backlog::no-commitment"` otherwise.
...
Technical debt is prioritized like [other technical decisions](/handbook/engineering/development/principles/#prioritizing-technical-decisions) in [product groups](/handbook/company/structure/#product-groups) by [product management](/handbook/product/product-processes/cross-functional-prioritization/).

For technical debt which might span, or fall in gaps between groups they should be brought up for a [globally optimized](/handbook/values/#efficiency-for-the-right-group) prioritization in [retrospectives](/handbook/engineering/careers/management/group-retrospectives/) or directly with the appropriate member of the [Product Leadership team](/handbook/product/product-leaders/product-leadership/). Additional avenues for addressing technical debt outside of product groups are [Strategic Priority Codes](/handbook/engineering/workflow/strategic-priority-codes/) and [working groups](/handbook/company/working-groups/).
...
These issues are prioritized like [other technical decisions](/handbook/engineering/development/principles/#prioritizing-technical-decisions) in [product groups](/handbook/company/structure/#product-groups) by [product management](/handbook/product/product-processes/cross-functional-prioritization/).

As with [technical debt](#technical-debt), Deferred UX should be brought up for [globally optimized](/handbook/values/#efficiency-for-the-right-group) prioritization in [retrospectives](/handbook/engineering/careers/management/group-retrospectives/) or directly with the appropriate member of the [Product Leadership team](/handbook/product/product-leaders/product-leadership/).
...
- [Production Engineering](https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/issues?label_name[]=infradev&state=opened)
- [Gitaly](https://gitlab.com/gitlab-org/gitaly/-/work_items?label_name%5B%5D=infradev&state=opened)
- [Runway](https://gitlab.com/gitlab-com/gl-infra/platform/runway/team/-/work_items?label_name%5B%5D=infradev&state=opened)
