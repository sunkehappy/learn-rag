---
title: "Editor Extensions: VS Code Group"
description: We own and maintain code editor extensions for VS Code, WebIDE as well as contributing to the Language Server bringing GitLab’s core features and AI capabilities directly into developer workflows.
---

**Note:** Along with the [Editor Extensions: Multi-Platform](/handbook/engineering/ai/editor-extensions-multi-platform/) team, we are responsible for all aspects of the product categories that fall under the [Editor Extensions group](/handbook/product/categories/#editor-extensions-group) of the [AI-Powered stage](/handbook/product/categories/#ai-powered-stage) of the [DevOps lifecycle](/handbook/product/categories/#devops-stages).

## Group overview

### Group members

{{< team-by-manager-slug "donaldcook" >}}

### Stable counterparts

The following people are [stable counterparts](/handbook/leadership/#stable-counterparts) of the AI-Powered:Editor Extensions group:
...
We have a sync meeting ([notes (internal)](https://docs.google.com/document/d/1UJg-Prf5qGjiGImvaYl5HNjMcJddoeE4u33Ri6SxQ6g)) as a team once per week alternating between AMER/EMEA and AMER/APAC time slots weekly.
...
- Our team and stable counterparts frequent the following Slack channels:
...
- Discuss specific features in their dedicated channels:
...
The [Ownership and Boundaries](/handbook/engineering/ai/editor-extensions-vscode/ownership/) page provides clarity and a clear expectation between all parties who author/maintain features in our systems.
...
- Alice is familiar with the system but is picking up an issue (weight `2`) that touches authentication. Alice is unfamiliar with our authentication patterns and adds `1` subjective weight to have enough time to review the existing approaches. That makes the final weight `3`.
...
We use [work item status](https://docs.gitlab.com/user/work_items/status/) when working with issues.

- `Planning breakdown` - the issue is ready for prioritization.
- `Ready for development` - the issue has been described, estimated and scheduled, and it's ready to be picked up and worked on.
- `In dev` - the issue has an assigned person who started implementing it.
- `In review` - the spike/implementation is finished, and someone needs to review the spike result or the last MR (the last MR because if there are more MRs to implement, only the last one should result in the change of the workflow label).

Use scoped `~workflow` labels when work item status is unavailable:
