...
See the [work section](/handbook/engineering/devops/create/code-review/#work) of the main Code Review page.
...
To facilitate the distribution of information across the team and increase transparency, we use announcement issues instead of Slack to share team-wide messages. These issues are sent whenever needed rather than on a fixed cadence.

Announcement issues are labeled with `~Announcements` and typically target backend engineers plus the Engineering Manager. The [announcement issue template](https://gitlab.com/gitlab-com/create-stage/code-review-be/-/issues/new?description_template=announcement) provides options to also notify the Product Manager and/or stable counterparts.

Anyone on the team can share announcements using this format. Minor or repetitive announcements may still be sent via Slack (e.g., milestone reminders for PTO).
...
The primary source for things to work on is the [Code Review Release board](https://gitlab.com/groups/gitlab-org/-/boards/2159734?milestone_title=Started)
...
[milestone planning process](/handbook/engineering/devops/create/code-review/backend/manager),
...
There is also the [Code Review backend _assignment_ issue board](https://gitlab.com/groups/gitlab-org/-/boards/2142016?milestone_title=Started),
...
You can find the issues assigned to you on the [Code Review backend assignment issue board](https://gitlab.com/groups/gitlab-org/-/boards/2142016?milestone_title=Started).
...
by [filtering the Code Review Release board](https://gitlab.com/groups/gitlab-org/-/boards/2159734?&assignee_id=None&milestone_title=Started)
...
controller actions available (i.e. not emitting errors/500s) and performant (i.e. below our latency targets).

#### Availability dashboards

We use the following dashboards to monitor and identify issues impacting our error budget and users of our features:

- [(Grafana) Code Review: group error budget detail](https://dashboards.gitlab.net/d/stage-groups-detail-code_review) (internal only)
- [(Sentry) Code Review Workflow - Users experiencing errors](https://new-sentry.gitlab.net/organizations/gitlab/dashboard/27/?dataset=events&project=3&statsPeriod=24h) (internal only)
- [(Sentry) MR Homepage - Users experiencing errors](https://new-sentry.gitlab.net/organizations/gitlab/dashboard/33) (internal only)

For root cause analysis, we explore if site-wide / infrastructure instability may be causing issues, by:

- Checking if other stage groups experience the same apdex/error patterns: [stage groups](https://dashboards.gitlab.net/dashboards/f/stage-groups/stage-groups)
- Checking system-specific dashboards for impact:
...
- Searching through recent incidents in [gitlab-com/gl-infra/production/-/issues](https://gitlab.com/gitlab-com/gl-infra/production/-/issues?sort=created_date&state=opened&first_page_size=100)

#### Monitoring automation

Alerts for events impacting our feature category are posted to the [#g_create_code-review_alerts](https://gitlab.enterprise.slack.com/archives/C082E78PJJK) Slack channel:

- Prometheus SLO violation alerts ([configuration](https://gitlab.com/gitlab-com/runbooks/-/blob/aab93b33baa1da414f4d03454f887c78d7222463/services/teams.yml#L253-262))
- Sentry alerts for users experiencing errors:
  - All Code Review Workflow errors ([configuration](https://new-sentry.gitlab.net/organizations/gitlab/alerts/rules/details/9))
  - MR homepage API errors ([configuration](https://new-sentry.gitlab.net/organizations/gitlab/alerts/rules/details/16))

Every Monday, the 7-day error budget for the past week is automatically posted in [#g_create_code-review](https://gitlab.enterprise.slack.com/archives/C01EMBKS5DW).

Issues are created automatically for the following database problems:

- [Table growth anomalies](https://gitlab.com/gitlab-com/gl-infra/capacity-planning-trackers/gitlab-com/-/issues/?state=opened&label_name%5B%5D=anomaly&label_name%5B%5D=group%3A%3Acode_review)
- [Table size approaching or above limits](https://gitlab.com/gitlab-org/gitlab/-/issues/?state=opened&label_name%5B%5D=database%3A%3Atable%20size&label_name%5B%5D=group%3A%3Acode%20review)

#### Performance dashboards

Kibana dashboards grouped by the endpoint/action and sorted by P90 (slowest first):
...
- [general: Application SLI Violations](https://dashboards.gitlab.net/d/general-application-sli-violations/general3a-application-sli-violations?orgId=1&from=now-1h&to=now&timezone=utc&var-PROMETHEUS_DS=mimir-gitlab-gprd&var-environment=gprd&var-environment-2=gprd&var-stage=main&var-product_stage=$__all&var-stage_group=code_review&var-component=$__all): apdex and error rate per endpoint
- [stage-groups: Code Review: Group dashboard](https://dashboards.gitlab.net/d/stage-groups-code_review/stage-groups-group-dashboard-create-code-review?orgId=1): more specific details about each actions and endpoints
...
- [Create:Code Review BE Engineer Resources](/handbook/engineering/devops/create/code-review/backend/engineers), e.g. team building and career development
- [Create:Code Review BE Engineering Manager Responsibilities](/handbook/engineering/devops/create/code-review/backend/manager), e.g. milestone planning, talent assessment, and project management
- [Create:Code Review AI prompts](/handbook/engineering/devops/create/code-review/ai-prompts/), common prompts we use to be more efficient.
