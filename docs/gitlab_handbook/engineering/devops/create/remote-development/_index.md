...
The group is part of [Create Stage](/handbook/engineering/devops/create/) in the [Dev Sub-department](/handbook/engineering/devops/). We focus on two [categories](https://about.gitlab.com/direction/create/#categories-in-create): `Workspace` and the `Web IDE`.
...
Team members should add any [planned time off](/handbook/people-group/time-off-and-absence/time-off-types/) in Workday, so that the Engineering Manager can use the proper number of days off during capacity planning.
...
Error budgets and availability metrics must accurately reflect the actual customer experience with Workspaces.
The focus here should be on behaviours that impact customers.

#### Learnings So Far

Our error budgets went red at different points in the past despite having monitoring in place.
We were reviewed dashboards weekly on a fixed schedule, but errors sometimes accumulated between reviews.
We had postponed setting up Sentry alerts because we wanted to avoid noise, but this "pull" approach meant we missed early warning signs that could have prevented impact.

**The key lesson:** it's better to start with noisy alerts that push issues to you immediately and refine them over time. This shift from reactive weekly reviews to proactive, real-time alerting has kept our dashboards consistently green for a long time now.

#### Customer Impact Validation

Prioritise alerts and errors that impact users' ability to:

- Create new workspaces
- Connect to existing workspaces
- Use workspaces with reasonable performance
- Maintain development flow
- Trust data persistence

#### Availability Champion

Each milestone, we designate an Availability Champion who owns operational health for ~4 weeks.

**This isn't on-call duty** - it's about being the team's eyes on our system health and ensuring nothing slips through the cracks.

**Responsibilities:**

- Monitor #f_workspaces_alerts channel
- Run the weekly operational health review every Monday
- Create issues for any error budget consumption
- Be the go-to person for "is this error important?" questions

#### Sentry Alerts in #f_workspaces_alerts
...
**Principle:** Strive to understand every minute of error budget spent.

- Check the weekly error budget notification in #g_remote_development every Monday and investigate consumption.
- React to the notification with ✅ to show review is done.
