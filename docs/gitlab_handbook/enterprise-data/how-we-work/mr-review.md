...
The number of code owners assigned to each code path should strike a balance between ensuring sufficient coverage and maintaining clear ownership. A code owner should have **deep knowledge** of their assigned domain to confidently evaluate changes, we prioritize domain expertise over team size. Code owners must be able to assess whether a change is technically sound and aligns with our standards.

As a general guideline, less complex code (such as dbt models) can accommodate a higher number of code owners (3-5), making reviews more accessible and distributed. More complex code (such as data extraction pipelines) requiring specialized knowledge benefits from a smaller group of code owners (2-3) to ensure thorough, high-quality reviews from domain experts.
...
Because of quality and [security](/handbook/security/policies_and_standards/gitlab_projects_baseline_requirements/#mr-approval-rule-configurations) we require every MR to be approved before merged with multiple Team Members involved. By default the CODEOWNER file sets that an approval is needed and by who. But if a file or folder is not present in the CODEOWNER file a MR still can be merged without approval or without other Team Members involvement. To enforce the following 2 settings must be applied on projects within the [GitLab-Data Group](https://gitlab.com/gitlab-data/) if it meets the criteria mentioned in [this handbook section](/handbook/security/policies_and_standards/gitlab_projects_baseline_requirements/#purpose).
