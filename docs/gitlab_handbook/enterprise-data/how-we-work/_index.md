...
Backlog grooming is the ongoing process of reviewing and cleaning up the backlog to keep it healthy and actionable. Regular grooming ensures we're focused on work that still provides business value. This is the collective responsibility of all members of the Data team. Team members should regularly review issues that are assigned to them / that they have created and close items that are no longer needed, have been superseded, are stale (not touched in 365 days with no active business need), or lack sufficient detail. Closing an item doesn't discard the work, if priorities change we can reopen it. Use your judgment and consult your manager or stakeholder when uncertain. Grooming happens throughout the year.

**Note:** An [automated triage policy](/handbook/enterprise-data/how-we-work/triage/#stale-issue-management) will automatically close issues that have not been modified for 365 days. The bot checks for any (technical) modifications in an issue, so actions like linking to an issue count as an update and will prevent closure.
...
- Subject to [Data Steering Committee](/handbook/enterprise-data/how-we-work/#data-steering-committee) review and approval
...
| [OKR-Level Project Request](https://gitlab.com/gitlab-data/analytics/-/issues) | `[Request] Create Opportunity Canvas` _(note: this template is required for large-scale, P2 work like OKRs to be reviewed and prioritized in the Data Steering Committee)_|
...
DataPulse is a monthly showcase for Central Data Team Members to provide status updates on [Level-3 and Level-4 Epics](/handbook/enterprise-data/how-we-work/planning/#quarterly-planning). The goal is for everyone to stay informed about project statuses, identify new opportunities and maintain alignment across our various workstreams.

We'll meet for a focused session to:
  
- Learn from each other and make it conversational
...
- **Presenters:** Each month, 3 team members are selected to share updates on their active Level-3 or Level-4 Epics.
- **Rotation:** We alternate presenters across teams to ensure broad visibility and cross-team awareness.
- **Schedule:** Sessions run monthly, alternating between the `IST/UTC` and `PST/EST` time slots to accommodate all time zones.
- **Recording:** Sessions are recorded so team members can catch up if the session time doesn't work for their timezone.
- **Format:** Presenters share their project during the session. Team members are encouraged to ask questions, provide feedback, and collaborate throughout.
...
1. Whether there is an SLO breach per [SLOs standards](/handbook/enterprise-data/platform/#data-sources)
2. Whether there is immediate impact on downstream models, business operations or data consumers
3. Whether immediate action is needed to resolve the incident
...
1. Whether there is immediate impact on data, downstream models, business operations or data consumers
2. Whether there is an SLO breach per [SLOs standards](/handbook/enterprise-data/platform/#data-sources)
3. Whether immediate action is needed to resolve the incident

For detailed incident definitions, severity levels, and creation procedures, see the [Data Team Incident Management](/handbook/enterprise-data/data-governance/incident-management/) handbook page.

#### Process for working through incidents

- Open an [Incident issue](https://gitlab.com/gitlab-data/analytics/-/issues/new?issuable_template=incident&issue[issue_type]=incident) using the "Incident Report" template. Alternatively, you can convert an existing issue into an incident if it meets the incident criteria. (Note: this can be done by adding a new line and typing `/incident` in the respective issue).
  - Note: Incidents related to the [Data Program](https://gitlab.com/gitlab-data/) are always created in the [Analytics project](https://gitlab.com/gitlab-data/analytics/) to maintain centralized incident tracking and management.
- Detail the relevant information with appropriate timestamps and [severity level](/handbook/enterprise-data/data-governance/incident-management.md#2-incident-severity).
- Tag and assign people on the Data Team and any other teams that need to be informed.
- For infrastructure issues requiring SRE collaboration, also use [incident.io](https://incident.io/) while maintaining a corresponding GitLab incident.

Data Team Incidents can be reviewed in [Incident Overview page](https://gitlab.com/gitlab-data/analytics/-/incidents) within the Analytics project.
...
- The handbook definition should be reviewed by the necessary [Consulted & Informed](/handbook/people-group/directly-responsible-individuals) cross-functional partners. In some cases, the definition may also require [approval](/handbook/people-group/directly-responsible-individuals) from those cross-functional partners
