...
- Ensure your SSH configuration is setup according to the [GitLab directions](https://docs.gitlab.com/user/ssh/). Your keys should be in `~/.ssh/` and the keys should have been generated with no password.
...
- **NB**: Before running dbt for the first time run `make prepare-dbt`. This will ensure you have venv installed.
  ...
  - If you get an error like this `ssh: connect to host gitlab.com port 22: Operation timed out fatal: Could not read from remote repository.`, and you've made sure you have correct access rights and the repository exists, then it could be an SSH issue where SSH connections to GitLab on port 22 were being blocked by network/firewall. To solve this, configure SSH to use GitLab's alternative SSH service on port 443.
    ...
    Update SSH config for GitLab. Change the port in ~/.ssh/config file.
...
Each production pipeline is defined as an individual Python file in the [`dags/transformation`](https://gitlab.com/gitlab-data/extract/-/tree/main/dags/transformation) directory of the `extract` repository. Schedules, dbt selectors, and Snowflake warehouse sizes are hardcoded within each file.

| DAG ID | Schedule | Description | Source |
|---|---|---|---|
| [`dbt_snapshots`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_snapshot.py) | Daily at 07:00 UTC | Runs all dbt snapshots, then builds and tests legacy snapshot models | `dbt_snapshot.py` |
| [`dbt_edm_snapshots`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_edm_snapshot.py) | Daily at 17:00 UTC | Runs EDM-tagged snapshots, then builds and tests EDM snapshot models | `dbt_edm_snapshot.py` |
| [`t_gitlab_customers_db_dbt`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/gitlab_dot_com_dbt.py) | Daily at 06:30 UTC | Runs snapshots, incremental model refresh, and tests for the GitLab customers database | `gitlab_dot_com_dbt.py` |
| [`t_gitlab_com_db_dbt`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/gitlab_dot_com_dbt.py) | Twice daily at 07:00 and 19:00 UTC | Runs snapshots, incremental model refresh, and tests for the GitLab.com database | `gitlab_dot_com_dbt.py` |
| [`dbt`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt.py) | Daily at 08:45 UTC | Incremental refresh of all product and non-product models, workspace models, and tests | `dbt.py` |
| [`dbt_full_refresh_monthly`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_full_refresh_monthly.py) | First Sunday of each month at 08:45 UTC | Full refresh of all dbt models | `dbt_full_refresh_monthly.py` |
| [`dbt_sfdc_validation_and_run`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_sfdc_validation_and_run.py) | Every 6 hours | Builds and tests SFDC source models and Salesforce opportunity models | `dbt_sfdc_validation_and_run.py` |
| [`dbt_netsuite_actuals_income_cogs_opex`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_netsuite_actuals_income_cogs_opex.py) | 4 times daily, Monday–Friday | Runs NetSuite actuals models on business days only | `dbt_netsuite_actuals_income_cogs_opex.py` |
| [`t_omamori_external`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_omamori_external_table_transform.py) | Hourly | Refreshes Omamori external tables and runs source models | `dbt_omamori_external_table_transform.py` |
| [`dbt_gdpr_delete_requests`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_gdpr_delete.py) | Daily at 03:00 UTC | Processes incremental GDPR deletion requests | `dbt_gdpr_delete.py` |
| [`dbt_company_scorecard`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_company_scorecard.py) | Twice daily at 08:00 and 20:00 UTC | Refreshes Company Scorecard dbt models after GitLab.com data is available | `dbt_company_scorecard.py` |

#### Future State (In Progress)

The orchestration is being migrated to a configuration-driven system as part of the [Standardize dbt Model Orchestration with Frequency-Based DAGs](https://gitlab.com/groups/gitlab-data/-/epics/1555) initiative. The goal is to make it easier to add and maintain pipelines, and to give individual dbt models ownership of their own refresh cadence.

The default assumption for all dbt models is that new data arrives daily, and therefore models run on a daily cadence. In addition to the standard processing frequencies, a monthly maintenance window runs on the first Sunday of each month. This maintenance run performs a full refresh of all incrementally-built models to recover from any data drift that may have accumulated over time. The monthly maintenance window takes precedence over all other processing frequencies — any DAG whose schedule overlaps with the maintenance window is suppressed for that run. This precedence is intentional: it avoids duplicate processing and ensures data quality is restored before normal cadence resumes.

**Configuration-driven DAG generation**

Rather than writing a new Python DAG file for each pipeline, job definitions are centralized in a single YAML manifest — [`dbt_jobs_manifest.yml`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_jobs_manifest.yml) — and a single Python processor — [`dbt_jobs_manifest_processor.py`](https://gitlab.com/gitlab-data/extract/-/blob/main/dags/transformation/dbt_jobs_manifest_processor.py) — reads the manifest and dynamically generates Airflow DAGs at parse time, one per job entry.

Each job entry in the manifest supports:

- **`schedule`** – A cron expression defining when the DAG runs
- **`exclusion_schedule`** – An optional cron expression for dates or days to skip (e.g. skip the first Sunday of the month for monthly maintenance)
- **`warehouse`** – The default Snowflake warehouse to use for the job
- **`tasks`** – An ordered list of dbt steps, each with a `selector`, optional `warehouse` override, and `resource_types` filter (e.g. `model`, `snapshot`, `seed`, `test`)

The `company_scorecard` pipeline is the first to be defined in the manifest, maintaining its existing schedule and behavior. The remaining pipelines listed above are planned to be migrated to the manifest over time, eventually replacing the individual DAG files.

An example job entry from `dbt_jobs_manifest.yml`:
...
**Model-level frequency configuration**

The longer-term vision is for each dbt model to declare its own refresh cadence using a `config.meta.frequency` value. This drives the construction of frequency-based dbt selectors in `selectors.yml`, which in turn map directly to job entries in the manifest — one job per supported frequency.

Supported frequency values are:

| Frequency | Description |
|---|---|
| `one_hour` | Refresh every hour |
| `four_hour` | Refresh every 4 hours |
| `six_hour` | Refresh every 6 hours |
| `eight_hour` | Refresh every 8 hours |
| `twelve_hour` | Refresh every 12 hours |
| `twenty_four_hour` | Refresh daily (default for all models) |
| `weekly` | Refresh weekly |
| `first_week_of_month` | Refresh during the first week of each month |
| `monthly` | Refresh monthly |

The default frequency is `twenty_four_hour`, meaning new models require no explicit configuration. A frequency can be set at the directory level in `dbt_project.yml` or overridden at the model level:
...
- Documenting and testing new data models is a part of the process of creating them. A new dbt model is not complete without tests and documentation. See [Documenting dbt Models](#documenting-dbt-models) for project conventions.
...
- Clone the target sample tables
  - Using a command such as [`clone-dbt-select-local-user`](/handbook/enterprise-data/platform/dbt-guide/#cloning-models-locally) ensure that there is a full data table to sample from.
...
Purpose: This test validates the account ID field in Zuora. This field is always 32 characters long and only has numbers and lowercase letters when it is not NULL.

Because we use dbt, we have documentation for all of our source tables and most of our downstream modeled data. With in the yaml documentation files, we're able to add tests to individual columns. See [Documenting dbt Models](#documenting-dbt-models) for conventions on writing model and column descriptions.
...
Documentation is a first-class requirement for every dbt model. As stated in the Style and Usage Guide: **a new dbt model is not complete without tests and documentation.** This section explains the project's conventions for writing and maintaining those descriptions consistently.

### Why documentation matters

Approximately 25% of models and fewer than 1% of source columns currently have descriptions. This gap has two concrete costs: developers must read SQL to understand a model's grain and business logic rather than finding that information in the catalog, and AI-assisted development tools (SQL generation, query answering) have little semantic context to draw on when descriptions are absent. Consistent, structured documentation addresses both.

### Conventions at a glance

| What to document | Convention |
|---|---|
| Model description | `{% docs model_name %}` block in the directory-level `.md` file; reference in `schema.yml` as `'{{ doc("model_name") }}'` |
| Shared column descriptions | `{% docs column_name %}` block in `common_columns.md`; reference in `schema.yml` as `'{{ doc("column_name") }}'` |
| Model-specific column descriptions | Inline prose in `schema.yml` (rare — only when the meaning is unique to that model) |
| Source table and column descriptions | Inline prose directly in `sources.yml` |
| New shared vocabulary entries | Always appended to the **end** of `common_columns.md` — never inserted mid-file |
| Logic carried from upstream | Prefix filter and business logic bullets with `` `Inherited` `` |
...
Every model description is written as a `{% docs %}` block using the following structure:
...
All five sections should be present for mart and fact models. For simpler prep or staging models, sections with nothing to say (such as **Other Comments**) may be omitted.

#### Where the doc block lives

Model descriptions are **never** written inline in `schema.yml`. Instead:

1. Write the `{% docs model_name %}` block in the **directory-level `.md` file** — the file named after the directory that contains the model's SQL file.

   | Model path | Directory `.md` file |
   |---|---|
...
2. If no `.md` file exists for the directory yet, create one named `<directory_name>.md` in that directory.

3. Always **append** new `{% docs %}` blocks to the end of the file. Never reorder or overwrite existing entries.

#### Referencing the doc block in schema.yml

Once the block is written in the `.md` file, reference it in `schema.yml`:
...
When a filter or piece of business logic originates in an upstream model and is not re-implemented in the current model, prefix the relevant bullet with `` `Inherited` ``. This tells readers the logic is not in this model's SQL and they should look upstream for its definition.
...
Only use `` `Inherited` `` for logic that is genuinely defined upstream. If you are unsure, read the model's SQL to verify.
...
For each column needing a description, follow this order:

1. **Check `common_columns.md`** (`transform/snowflake-dbt/models/common/common_columns.md`). Search for `{% docs column_name %}`. If found, use `'{{ doc("column_name") }}'` in `schema.yml`. Done.

2. **Check upstream `schema.yml` files.** If an upstream model already has a description for this column, that description belongs in `common_columns.md`. Add it there, then use `'{{ doc("column_name") }}'` in both the upstream and current `schema.yml`.

3. **No description exists anywhere.** Write a concise prose description, register it in `common_columns.md`, then use `'{{ doc("column_name") }}'` in `schema.yml`.

#### Adding to `common_columns.md`

New entries are appended to the end of `transform/snowflake-dbt/models/common/common_columns.md` using this format:
...
- The `{% docs %}` and `{% enddocs %}` tags each appear on their own line.
- One blank line between the tag and the description body, and between the body and `{% enddocs %}`.
- Always append to the end of the file. Never insert mid-file or overwrite an existing entry.
- Before adding, confirm the entry does not already exist by searching for `{% docs column_name %}`.
...
#### When to use inline prose

Use inline prose (not a `{{ doc(...) }}` reference) only when a column's meaning is genuinely specific to one model and would never apply elsewhere — for example, a derived column whose interpretation depends entirely on that model's specific grain or join logic. This should be the exception. When in doubt, register the description in `common_columns.md` and reference it.

---

### Source definitions

Source table and column descriptions are written **inline** directly in `sources.yml`. Do not use `{{ doc(...) }}` references or separate `.md` files for sources.
...
When writing source descriptions from external documentation (vendor API docs, source system field glossaries), adapt to match project style: concise, present tense, factual.

---

### The `/docs` command

The `/docs` command is an AI-assisted workflow that automates the documentation enrichment process for dbt models changed on the current branch.

#### What it does

When invoked, `/docs` determines which `.sql` model files have changed on the current branch relative to `master` (including uncommitted changes and new untracked files), then applies the `dbt_enrich_descriptions` skill to each changed model. For each model it:

1. Audits the existing `schema.yml` entry to identify missing model and column descriptions.
2. Reads the model SQL and traces column origins through the lineage.
3. Generates a structured five-section model description and appends it to the directory-level `.md` file.
4. Generates column descriptions following the decision tree above — checking `common_columns.md` first, then upstream `schema.yml` files, then generating new entries.
5. Updates `schema.yml` with `{{ doc(...) }}` references and appends any new entries to `common_columns.md`.

The command never modifies SQL files. It only touches `.yml` and `.md` files.

#### How to use it

In the [opencode](https://opencode.ai) TUI, type:

```plaintext
/docs
```

No arguments are required. The command automatically detects changed models on the current branch.

To document a specific model without running `/docs`, open opencode and describe the task directly:
...
The AI generates descriptions as drafts. Before committing, review:

- **Model description accuracy** — confirm the five-section template reflects the actual SQL logic, especially Data Grain, Filters, and Business Logic sections.
- **`Inherited` usage** — verify that logic marked as `Inherited` is genuinely defined upstream and not re-implemented in the model's SQL.
- **New `common_columns.md` entries** — confirm new entries accurately describe the column and are not duplicates of existing entries under a slightly different name.
- **`{{ doc(...) }}` references** — confirm every reference points to an entry that now exists in `common_columns.md` or the directory `.md` file.

Documentation generated by `/docs` follows the same review process as any other MR change. The command accelerates the first draft; the engineer is responsible for accuracy.

---

### Examples

#### Full model description example

The following is the description for `mart_event_valid` in `models/common_mart/common_mart.md`:
...
#### Full column description example

An entry in `common_columns.md`:
...
Referenced in `schema.yml` across any model that exposes this column:
...
Take note of how we [talk about and define snapshots](/handbook/enterprise-data/platform/#snapshots).
...
### Snapshot MR Template

For step-by-step development instructions, use the [`dbt_snapshot_changes.md` MR template](https://gitlab.com/gitlab-data/analytics/-/blob/master/.gitlab/merge_request_templates/dbt_snapshot_changes.md?ref_type=heads).
...
When data needs to be backfilled, for example on an incremental model, to the most common approach is to perform a full refresh, `--full-refresh` on the model. This is done on the first Sunday of each month, to minimize data drift, by the `dbt_full_refresh_monthly` DAG, or can be performed manually with the `dbt_manual_refresh` DAG if there is a need to reload the data outside of the monthly cadence. No specific configuration is needed on an incremental model for these backfill methods for function and should be considered the default approach.

Should the data prove to be to large for this kind of backfill, the query times out or in the monthly refresh, generally takes more than an hour to refresh or if there is no business/technical need to monthly backfill, then the model can be set to not refresh when the `--full-refresh` flag is passed but configuring the models `full_refresh` property to use the `only_force_full_refresh()` macro.  This macro will return false, forcing the model to always run in its incremental mode, unless the `--full-refresh` flag is set and the `full_refresh_force` variable is set to `true`.  This allows us t programmatic enable a full refresh manually if is needed without changing the configuration of the model.

Should the option of backfill data on a large incremental model still be needed, and the full refresh has been disabled for taking too long, then the model can be set up to allow for what we call an incremental backfill.  

To set up the incremental backfill in the model, add the incremental_backfill_filter('date_filter_column', 'date_check_column') macro in the appropriate location within the WHERE clause. This macro applies a time period filter to the query, based on whether the incremental_backfill_start_date and incremental_backfill_end_date variables are provided at runtime.

If the variables are not included then the macro will default to the max of the passed date check column and one month into the future from that date.  These variables can be passed manually, `{"incremental_backfill_start_date": "2025-09-01", "incremental_backfill_end_date": "2025-09-08"}'` and are used by existing backfill DAGs.  For more details information on the `incremental_backfill_filter` please see the dbt Docs page for the macro.
...
Snapshot data cannot be recovered if lost in Snowflake. On a case-by-case basis, team members should evaluate whether to offload critical snapshot data using the [archive process](./#backups) as a backup measure.
...
- Check data source [guidelines from the handbook](/handbook/enterprise-data/platform/).
