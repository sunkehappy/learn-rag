...
## Embedding Tableau Charts in Google Docs and Slides

### Overview

The Tableau for Google Workspace add-on allows you to embed Tableau visualizations into Google Docs and Slides as static images that link to live dashboards. The add-on also supports automatic link unfurling and bulk refresh capabilities.

#### Video Walkthrough

Watch a demonstration of how to use the Tableau add-on. Internal link [here](https://drive.google.com/file/d/1W7mFW1XZ0APPFqYyexmX3O1Ko5mfvWKo/view?usp=drive_link).

### Installation (One-Time)

1. For Tableau users, the extension should already be installed automatically by IT. If not already installed, install the [Tableau for Google Workspace add-on](https://workspace.google.com/marketplace/app/tableau/1044537977580)
2. Once configured, any licensed Tableau user can use the add-on

### Using the Add-on

**In Google Docs or Google Slides:**

1. Open **Extensions** → **Tableau** → **Start**
2. Search for Tableau content or paste a Tableau URL
3. Click **Insert** to embed the image
4. Images are hyperlinked to the live Tableau view

*Note: To embed only a specific chart from a Tableau dashboard into a Google Doc, you must first publish the individual worksheet separately, not as part of the full dashboard. Then, you can publish the individual worksheet to Tableau Server or Tableau Cloud and use the "Share" option to get a link for embedding, which can then be inserted into Google Docs or Slides. The original prompt also had a spelling error: "separaetly" should be "separately" and "specicic" should be "specific".

**Smart Features:**

- **Link Unfurling**: Paste a Tableau Cloud URL and it automatically creates a Smart Chip (Docs) or Preview Link (Slides)
- **Refresh Images**: Click **Refresh All** in the add-on to update all embedded Tableau images at once
- **Refresh Tracking**: Refresh results are logged in document comments with timestamps

**Current Chart Refresh Options (Manual Only):**

1. Manual Individual Refresh: Click on a specific image and refresh it through the add-on
1. Manual Bulk Refresh: Click "Refresh All" button in the add-on sidebar to update all Tableau images in the document at once

### Best Practices

- **Before Sharing**: Refresh all Tableau images to ensure current data
- **Limit Embeds**: Keep to 3-7 images per document for optimal performance
- **Use Captions**: Add context like "Last refreshed: [Date]" below images
- **Choose Wisely**: Embed specific worksheets rather than complex full dashboards.

### Use Cases

- **Weekly Reports**: Embed key metrics and use bulk refresh every Monday to update all charts simultaneously
- **Presentations**: Build template decks with Tableau images, refresh before meetings to show latest data
- **Documentation**: Include Tableau views in process docs with hyperlinks for deeper exploration
