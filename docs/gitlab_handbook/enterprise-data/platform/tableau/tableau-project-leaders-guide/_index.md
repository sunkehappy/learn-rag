...
2. **Validating and Approving Content**:
   * **Ownership and Accountability**: Project Leaders have full ownership of their department's Production content and are responsible for determining what should be published or updated. They have the permissions to move content from Development to Production in Tableau at their discretion.
   * **Publishing Without an Issue**: Project Leaders can publish new content or update existing Production content at any time without creating a GitLab issue. For important updates that require visibility, stakeholder awareness, or explicit approval, it is recommended to open a [Tableau GitLab Issue](https://gitlab.com/gitlab-data/tableau/-/issues/new) using the `Tableau Publish to Production` template.
   * **Revision History as a Safety Net**: Tableau maintains full revision history for all workbooks, which provides a reliable fallback if a change needs to be rolled back.
   ...
   * **Single Source of Truth**: Ensure usage of a single source of truth (SSOT) for Certified and Published data sources. Also ensure that production content has one place for users to access trusted dashboards and reports. Minimize redundant or conflicting workbooks that might confuse end users.
...
1. **Sub-Folder Creation:** Project Leads have the ability to **create sub-folders** within their project folders. Sub-folders can help better organize content, especially in larger projects. Please **Inform the BI Team** when creating sub-folders to ensure that **permissions are set up properly** and are consistent with the overall structure of the project folder.
...
Project Leaders have the ability to promote workbooks to Production at their discretion and are responsible for the content they publish. After reviewing and validating the content, Project Leads can fulfill promotion by completing the following:
