...
- Order columns in SELECT statements with non-aggregated columns first, followed by aggregated columns:
...
- When writing `GROUP BY` clauses in dbt models, prefer using an approach that makes aggregations **explicit and intentional**. This can be achieved by using the [group_by dbt macro](https://github.com/dbt-labs/dbt-utils?tab=readme-ov-file#group_by-source) as well as explicit or positional column references. **Avoid** [GROUP BY ALL](https://docs.snowflake.com/en/sql-reference/constructs/group-by#label-group-by-all-columns), since it can hide important changes in aggregation logic.
