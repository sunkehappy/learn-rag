...
A [Sample Report](https://analytics.google.com/analytics/web/#/analysis/a37019925p267713722/edit/J2l0ms7fQ8OsqLLxLlHC4g) is available as a starting point for building custom reports. The template includes five pre-configured tabs:

* Landing Page: Sessions, engagement rate, and bounce rate by hostname and page path (filtered to `session_start` events)
* Page: Page views, scroll depth (75%), and exits by hostname and page path
* Conversion: Event counts, sessions, and total users for conversion events (`saas_trial`, `form_submit`, `purchase`)
* Page Conversions: Page performance with session-scoped segments comparing all users vs. conversion events (`saas_trial`, `form_submit`, `purchase`)
* Clicks: CTA click tracking showing page path, CTA location, CTA name, and total users (filtered to about.gitlab.com and `cta_click` events)

To use the template, make a copy of the Sample Report and customize dimensions, metrics, and filters as needed. Note the green checkmark or exclamation icon in the top right corner, which indicates whether the report is using sampled or unsampled modeled data.
