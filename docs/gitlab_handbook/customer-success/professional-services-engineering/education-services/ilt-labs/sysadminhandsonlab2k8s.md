...
1. On your instance, run the command `kubectl get pods`. You will see an output similar to below:
...
1. Let's try deploying our values file to verify that these values deploy successfully. To do this, run the command `helm upgrade gitlab gitlab/gitlab --version 9.4.3 -f values.yml`.
