...
1. First, let's add our token securely to our project as a project-level variable. Navigate to **Settings > CI/CD** and expand the **Variables** section.

1. Select **Add variable**. In the pane on the right, type GITLAB_RUNNER_TOKEN into the **Key** field, and paste your runner token into the **Value** field.

1. Set the **Visibility** of your variable to **Visible**. This will make sure that the value written to the `config.toml` file is not set to **[MASKED]**, which is the value printed out for masked variables.
