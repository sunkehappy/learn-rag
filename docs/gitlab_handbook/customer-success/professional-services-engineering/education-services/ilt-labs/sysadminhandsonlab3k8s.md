...
1. To be able to access these buckets, you need to provide credentials for an AWS service account. To do this, run the command `kubectl create secret generic storage-config --from-file=config=storage.config`.

1. After adding this configuration, run the command `helm upgrade --install gitlab gitlab/gitlab --version 9.4.3 --timeout 100s -f values.yml`.
