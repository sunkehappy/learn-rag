...
1. At the top of the page, you will see your full GitLab version. For this lab, you will see `18.4.3-ee`.
...
1. For this lab, locate version `18.5.1-ee.0` (associated with helm chart version `9.5.1`). Take note of this version, we will be upgrading to this version of GitLab.
...
1. Input your current GitLab version (`18.4.3`) and your target version (`18.5.1`).

1. Ensure that **Edition** is set to **Enterprise**, and Distro is set to **Ubuntu**.

1. Select `Go!`.

In the upgrade path, you will see only a single step is required, which is to install `18.5.1`. Let's complete this process.
...
1. To validate your version, in the left sidebar, select **Help**. In the resulting menu, you should now see version `18.5.1`.
