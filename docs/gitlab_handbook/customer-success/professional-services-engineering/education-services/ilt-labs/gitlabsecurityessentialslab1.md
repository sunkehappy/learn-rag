...
1. Add an appropriate commit message (ex. `Add Advanced SAST functionality`). Ensure the **Commit to a new branch** option is selected. Set the branch name to `sast-update` and ensure that **Create a new merge request with this change** is checked. Click the **Commit changes** button.
