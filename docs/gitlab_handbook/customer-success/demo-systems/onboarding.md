...
1. It is out of scope for onboarding but you could continue to modify the agent, configure [user impersonations](https://docs.gitlab.com/user/clusters/agent/ci_cd_workflow/#impersonate-the-agent) using Kubernetes API as you add more and more projects to this group. The below example is what your manifest definition might look like if you started adding multiple projects:
...
- In your gcloud shell run `kubectl logs -f -l=app=gitlab-agent -n gitlab-agent-primary-agent` to see if any errors were logged
