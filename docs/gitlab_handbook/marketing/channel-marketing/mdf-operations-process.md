...
1. Fill in the `Operations Checklist.`
1. Once complete, request your operational support DRI move the issue from [Plan to WIP](/handbook/marketing/field-marketing/#process-for-moving-events-from-plan-to-wip). 
...
#### Step 4: Add Issue Link to the comment section of Zip
...
## Troubleshooting MDF Page Access

If a partner encounters a 404 error when trying to access the MDF page, this indicates insufficient permissions. To resolve this issue, verify the following requirements are met:
Account Level:

- Partner Status must be set to "Authorized" (contact the ESM if this needs to be changed, as partner status should not be modified directly)

User Level:

- Administrative Privileges must include "MDF Administrator" access

Both conditions must be satisfied for successful MDF page access. If the 404 error persists after confirming these settings, escalate to the #MktgOps slack channel for further investigation.
