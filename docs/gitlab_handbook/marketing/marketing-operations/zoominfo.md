...
ZoomInfo is accessed through Okta. Team members can request ZoomInfo access via [Lumos](https://app.lumosidentity.com/app_store).

Once you receive access to the Zoominfo platform you will need to download the [Zoominfo ReachOut Chrome extension](https://chrome.google.com/webstore/detail/zoominfo-reachout/fofjcndophjadilglgimelemjkjblgpf). You can also find the link by navigating down to the bottom right of your homepage.
...
| [Save and Subscribe Function Explained](https://help.zoominfo.com/s/article/Save-and-Subscribe-Function-Explained) |The ZoomInfo Sales Save and Subscribe function protects users from having to rebuild their lists when returning to the platform.|
...
ZoomInfo admin management is shared between the Revenue Technology team, who manages licenses for Field Sales roles (AEs/SAEs/ASMs/ISRs), and the Marketing Operations team, who manages licenses for the Sales Development Org (BDAs/SDRs/BDRs).
ZoomInfo admin management is shared between the Revenue Technology team, who manages licenses for Field Sales roles (AEs/SAEs/ASMs/ISRs), and the Marketing Operations team, who manages licenses for the Sales Development Org (BDAs/SDRs/BDRs).
...
The Account object in SFDC is owned by **Sales Operations**.

All SFDC accounts are currently enriched and re-enriched with ZoomInfo on a daily basis, with Instant Enrich turned on to enrich any new accounts upon creation.

Sales Operations is actively working to migrate account enrichment to **RingLead Enrich Premium**. Once the migration is complete, account enrichment will no longer run through the ZoomInfo scheduled job but will instead be handled by RingLead.

## SFDC Lead Enrichment

All leads in SFDC are enriched on a monthly basis on the third Friday.

In addition to leads beeing enriched on a monthly basis, we have just turned on Instant Enrich for Leads and all net new leads are getting enriched as they reach SFDC.
...
## Allow Lead Duplicate Creation

Previously, the Zoominfo Chrome extension blocked lead exports when a matching lead already existed in Salesforce. In duplicate scenarios, reps couldn’t import directly from the ZoomInfo Chrome extension and instead had to create leads manually, which slowed sequence enrollment and added unnecessary friction to their workflow.

We’ve now enabled ZoomInfo to create duplicate leads in Salesforce and rely on a [RingLead deduplication job](https://gitlab.ringlead.com/deduplication/96556301/general/) that runs daily at 3 PM ET to identify and merge new duplicates.

The toggle for this lives in the ZoomInfo Salesforce integration settings (under the lead export/duplicate handling configuration) where “Allow Lead Duplicate Creation” can be turned on. This keeps the Chrome extension useful in the scenarios reps care about most—quickly pulling in fresh enrichment and getting prospects into sequences immediately, while the scheduled cleanup maintains data quality and keeps duplicate volume under control.
