...
## SMB Advocate Email Escalation Process

When customers or prospects reach out via email and require assistance from an SMB Advocate, follow this process to ensure proper case creation and routing. The automated routing will consider regional coverage (AMER/EMEA/APJ)

Process Steps:

- Forward the email to **smallbusiness@gitlab.com**
- Automatic case creation *(Traction will attempt to attach the relevant account and contact before routing the case)*

Important Notes

- Always forward the entire email thread to ensure Advocates receive full context
- Do not reply to the customer before forwarding - let the system create and auto-assign the case first
- Manual intervention is only needed if the case cannot auto-assign (HVS Ops will intervene and assign the case)

Potential Scenario

**Email Attached to an Existing Case**: Please note that if the email thread is currently attached to an existing case in SFDC *(check the Chatter feed of the existing case to confirm)*, attaching the smallbusiness alias will not create a net new Case.

- If this happens, navigate to the aforementioned Account/Opportunity/Contact record in SFDC and create the SMB Sales Case
...
If a Customer needs support, they can reach our Support team by filing a ticket [here](https://about.gitlab.com/support/).

Internal GitLab team members can find further instructions on how to work with our GitLab Support team [here](/handbook/support/internal-support/#overview).
