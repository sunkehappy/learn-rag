...
- [Interpreting release dates](/handbook/engineering/releases/monthly-releases/) clarifies how product teams use milestones and labels to indicate the likelihood of feature delivery within certain time frames.
...
- Recording a quick video and sharing with Customer Success that discusses direction updates. Use sync meetings [as needed](/handbook/company/culture/all-remote/asynchronous/#gitlab-experts-advise-on-when-to-use-sync-vs-async) to facilitate efficient communication.
...
When writing about a feature, make sure to cover [these messaging guidelines](https://docs.gitlab.com/development/documentation/release_notes/)
...
feature. Describe the pain points and connect back to our [Value Drivers](/handbook/sales/command-of-the-message/#customer-value-drivers) (in this case `Reduce Security and Compliance Risk`).
...
##### Feature Change Locks (FCLs)

A feature change lock shifts a team's focus to reliability work for a timeboxed duration following high severity incidents. When an FCL is enacted, it has potential of impacting existing priorities. The FCL process is driven by engineering. See the detailed process in the [engineering page](/handbook/engineering/#feature-change-locks)
...
- Before rolling out a new service to support a major new feature launch, ensure that your team has gone through the [PREP process](/handbook/engineering/infrastructure-platforms/production/prep/). The effort and timing for a readiness review will vary depending on the complexity of the feature. It is recommended to start this process as early as practical when a significant number of the questions can be answered but not too late to further develop the feature based on learnings from the review.
- Ensure there are [application limits](/handbook/product/product-processes/#introducing-application-limits) for your product areas enabled on GitLab.com to reduce [abuse vectors](/handbook/security/security-operations/trustandsafety/abuse-on-gitlab-com/#abuse-categories).
...
As a product manager, you will be assigned as the [stable counterpart](/handbook/company/structure/#specialist) to a single [group](/handbook/company/structure/#product-groups). At GitLab we abide by
unique, and extremely beneficial guidelines when interacting with our groups. These include:

1. Product managers are the [DRIs](/handbook/people-group/directly-responsible-individuals/) for overall work prioritization but work collaboratively with their EM and UX stable counterparts to ensure the right priorities from each [work type](/handbook/product/groups/product-analysis/engineering/metrics/#work-type-classification) are considered as each has a different DRI. Product Managers are responsible for communicating overall priority.
1. Product Managers provide the what and when for feature work. Engineering (UX, Backend, Frontend) provide the how. This process is documented as part of our monthly [product](/handbook/product-development/how-we-work/product-development-flow/), [engineering](/handbook/engineering/workflow/#product-development-timeline) and [UX](/handbook/product/ux/product-designer/) cadence. We [define stable counterparts for each of these functions](/handbook/product/categories/) within a group.
...
As a company we emphasize [velocity over predictability](/handbook/engineering/development/principles/#velocity). As a product manager this means
...
(remember [everyone can](/handbook/company/mission/#mission)
contribute)
...
- [Strategic Priority Codes](/handbook/engineering/workflow/strategic-priority-codes/) - use when critical situations require immediate attention from various stakeholders across the Engineering Division
...
- [Realignment](/handbook/people-group/promotions-transfers/#department-transfers-manager-initiated) - use when a permanent assignment to a team is required to resolve ongoing challenges. This has the highest impact to team members and should be considered if other options cannot achieve the desired goal. We strive to hire team members in the groups that will need them most.
...
For each feature or category, we recommend tracking the improvements required to advance to the next [development stage](https://docs.gitlab.com/policy/development_stages_support/).
...
PMs should also feel free to collect and evaluate customer feedback independently. Looking at [existing](/handbook/product/ux/experience-research/finding-existing-research/)
...
then consider leveraging the [UX Research team](/handbook/product/ux/experience-research/problem-validation-single-stage-group/).
...
1. Use the USAT+ responder outreach [Glean email prompt - internal only](https://docs.google.com/document/d/1k_bnEoDnK7v30G6nnd9Jq740dpGrE_2oPGUR_6MNrfM/edit?usp=sharing) (see Using Glean to draft USAT+ outreach emails below) to generate your email. If you prefer to write from scratch, you can adapt the example email copy below.
...
##### Using Glean to draft USAT+ outreach emails

To keep outreach consistent and personalized, use our USAT+ responder outreach Glean email prompt when drafting emails to survey respondents who opted in to follow-up.

1. Open up the [USAT+ responder outreach Glean email prompt Google Doc - internal only](https://docs.google.com/document/d/1k_bnEoDnK7v30G6nnd9Jq740dpGrE_2oPGUR_6MNrfM/edit?usp=sharing)

1. Copy the full prompt into Glean and follow its instructions, answering the short set of questions about:
   - whether you want email-only feedback or a 25–30 minute call,
   - whether this is the first email or a second (and final) follow-up,
   - your role/product area,
   - the respondent’s first name and 1–3 key points from their USAT+ response,
   - the main outcome you want from the email.

1. Glean will generate:
   - three subject line options optimized for open/response rates, and
   - a draft email body you can review, edit for tone, and send.

1. Ensure your final email still:
   - clearly references the user's USAT+ survey feedback,
   - respects our limits of one GitLab team member contact per user and no more than two outreach emails (initial + one follow-up),
   - aligns with Product Legal guidance and the SAFE framework.

You can still use the example email copy in the sections above as a reference or fallback if you prefer to write your own email manually.
...
1. Use the USAT+ responder outreach [Glean email prompt - internal only](https://docs.google.com/document/d/1k_bnEoDnK7v30G6nnd9Jq740dpGrE_2oPGUR_6MNrfM/edit?usp=sharing) (see Using Glean to draft USAT+ outreach emails above) to generate your email. If you prefer to write from scratch, you can adapt the example email copy below.
...
1. See our training materials on [facilitating user interviews](/handbook/product/ux/experience-research/facilitating-user-interviews/).
