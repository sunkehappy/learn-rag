...
the Enterprise Data & Insights organization and serves as an analytics team to support
the GitLab Product Organization, in addition to product data-related analysis across GitLab.
...
The PDI team takes two different priorities into account when scheduling work, each captured
by a scoped label:
...
PDI priority is owned by the individual analyst. The team manager will help refine priority
based on importance and capacity. The team will work with the Enterprise Data & Insights
leadership and/or Product leadership on trade-offs (if needed).

PM priority is an input in determining PDI priority. However, the scope of the team's work
extends beyond section support (ex: cross-functional initiatives), and therefore there are
...
| Label | Priority |
| ----- | -------- |
| `pdi-priority::1` | **High / Urgent Priority:** Any analysis requests that are required to be completed within the current iteration and/or as soon as the data is available. All requests that have Priority 1 should have a direct KPI impact and/or executive visibility. Examples include metrics for feature launches, executive-level dashboards, and board metric validation. |
...
As GitLab team members, we are [encouraged to take PTO](/handbook/people-group/time-off-and-absence/time-off-types/) and observe public
...
Product Data Insights groups work into two different buckets. During planning, analysts will
aim to maintain the following breakdown:

- Operating Model priorities: 50%
  - Backlog DRI: Team manager and Enterprise Data & Insights leader (if trade-offs are needed)
- Other ad hoc work: 50%
  - Backlog DRI: Product analyst

*The distribution of work will vary from iteration to iteration, but the 50/50 breakdown
is the target state.*

### Urgent issues

If an urgent matter comes up, please open an issue and tag your analyst contact (and/or
`@dpeterson1`). Please include why the issue is urgent, when it is needed by, what it will
inform or how it will be used, and who is the intended audience.
...
and feel free to tag `@Dave Peterson`.
