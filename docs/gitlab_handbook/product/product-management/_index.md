...
Generally speaking, all product feedback should be provided via issues. If you have any product-related questions, comments, input, or otherwise, the Product Manager is the primary person you should talk to, *if creating an issue does not suffice*.
...
If a customer has a feature request that doesn't already exist, refer to the [process to create an issue](https://docs.gitlab.com/user/project/issues/create_issues/) in the [gitlab-org issue tracker](https://gitlab.com/gitlab-org/gitlab/-/issues) and choose the [Feature Proposal](https://gitlab.com/gitlab-org/gitlab/-/issues/new?issuable_template=Feature%20Proposal%20-%20lean) template, following the instructions and providing as much information as possible. Once you've created the issue,  make sure to add the appropriate labels for the [product stage and/or group](/handbook/product/categories/) (e.g `~"devops::plan"`) if known and add a comment tagging the appropriate Product Manager. If an issue already exists, make a comment on the issue with their information and use case.
...
the product is developed with [directly responsible individuals](/handbook/people-group/directly-responsible-individuals/) from Product, UX, and Development working together.

| Product Managers | Engineering Managers | UXers |
| :--------------: | :------------------: | :---: |
| Set [milestone priorities](/handbook/product/product-processes/cross-functional-prioritization/#planning-for-the-milestone) and define what features Engineering works on | Own the definition of done; decide what gets merged into Product. Prioritizes maintenance work. Engineers own quality end-to-end, including test strategy and requirements | Proactively identify small and large strategic UX needs to aid Product Management prioritization |
...
As the GitLab product matures, we know we must make important workflows easier to use through feedback-loop mechanisms as is captured in the "Improve" section of the [Product Development Flow](/handbook/product-development/how-we-work/product-development-flow/#outcomes-and-activities-7). We can use the [UX scorecards](/handbook/product/ux/ux-scorecards/) as mechanisms to provide insights into how might be able to improve these user workflows.

#### What if there is a conflict with the product direction plan and solution proposal?

For areas with low maturity, iteration and quickly adapting the product is the priority. In cases where the product experience desired would take longer to implement than required for the current maturity level, it is advised the Product Manager work with the Product Designer and/or Engineering Manager to scope an iteration plan to ensure the experience is delivered incrementally over time to provide value quickly with quality.

For areas with more adoption, or of higher maturity, we recommend using the below escalation path if there is a disagreement on the approach to solve for the product direction and experience for users/customers.
...
If there isn't a designer listed for a group, then that team is generally expected to be self-sufficient in taking responsibility for the design needs of their product area. For more complex situations, they can [request support from the UX team](https://gitlab.com/gitlab-com/Product/-/work_items/new?description_template=UX-Support-Request).

Helpful resources to aid with the decision-making are our [Pajamas Design System](https://design.gitlab.com/), our [UX Principles](https://design.gitlab.com/get-started/principles/) and the [Product Principles](/handbook/product/product-principles/). Product Design does not have the capacity to review complex proposed design solutions or provide design solutions for unsupported groups.
