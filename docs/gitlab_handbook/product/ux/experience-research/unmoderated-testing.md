...
At GitLab, we use [UserTesting.com](https://www.usertesting.com/) for certain unmoderated testing for [Solution Validation](/handbook/product/ux/experience-research/solution-validation-and-methods/). Unmoderated usability testing allows us to obtain evaluative feedback from our users very quickly, thereby allowing us to iterate faster in a more independent manner that requires fewer resources.
...
- **What if I want to ask problem validation related questions?** UserTesting.com isn't your best option. Instead, go through the [recruitment process](/handbook/product/ux/experience-research/recruiting-participants/) to identify participants in other ways.
...
- **How do I get an account?** Request via [Lumos](https://app.lumosidentity.com/app_store), which is accessible via Okta. UserTesting.com seats are allocated only to Product Designers and UX Researchers at this time, since those are the disciplines that are closely involved with solution validation testing.
