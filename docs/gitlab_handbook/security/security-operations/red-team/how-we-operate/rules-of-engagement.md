...
- The Red Team will make an effort to avoid triggering on-call incidents outside of normal business hours. In general, this means not conducting attack techniques during hours when it is a weekend or holiday globally. If the goal of an operation is to specifically test out-of-hours processes, this needs to be approved.
...
1. If an infrastructure incident is required, [engage the on-call SRE](/handbook/engineering/infrastructure-platforms/incident-management/#reporting-an-incident)
1. Perform a proper [root cause analysis](/handbook/engineering/workflow/root-cause-analysis/) following resolution of the incident
...
- Attempt to gain access to company-issued laptops using techniques like [drive-by attacks](https://attack.mitre.org/techniques/T1189/), [spearphishing with attachments](https://attack.mitre.org/techniques/T1566/001/), and [supply chain compromises](https://attack.mitre.org/techniques/T1195/). Specific [exclusions to this technique](https://internal.gitlab.com/handbook/security/security_operations/red_team/private_roe/#laptop-exclusions) are listed in the internal handbook.
...
- Other forms of social engineering that impersonate GitLab team members, vendors, customers, partners, etc. and are directed towards GitLab owned or managed services such as email, business phone numbers, authentication providers, job postings, etc. where the social engineering attempts do not directly compromise personal devices or services.
...
- Attempt to socially engineer team members by targeting their personal email addresses or social networks not affiliated with their GitLab employment.
- Directly call or SMS team member's personal phone numbers, except in the case where these numbers are shared publicly for GitLab work-related purposes.
