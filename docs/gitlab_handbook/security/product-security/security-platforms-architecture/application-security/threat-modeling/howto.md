...
### Consider Abuse and Platform Misuse

Features or products that introduce or expand access to free resources — compute minutes, storage, trial extensions, rate limit increases, or any form of quota allocation — carry an inherent abuse surface. Bad actors actively probe for ways to acquire resources at scale without paying. When the feature being reviewed touches any of the following, consider whether it could be abused for resource acquisition, cost avoidance, or platform manipulation:

- Free tier limits or trial eligibility logic
- CI/CD pipeline resources (compute minutes, runners, parallel jobs)
- DAP feature flows that could allow AI token or LLM quota abuse (e.g. proxying or redirecting allocated inference credits to third-party or unintended LLM usage)
- Invitation or referral mechanisms with resource side-effects such as CI minutes, project/namespace creations or DAP credits/trials

If you identify potential for abuse in any of these areas, loop in the Trust and Safety team early. Reach out via `#abuse` on Slack or tag `@gitlab-com/gl-security/security-operations/trust-and-safety` in your issue or MR. They can assess the abuse risk, work on detection signals, and coordinate a post-launch plan if required.
...
The threat model will be added to [the AppSec Threat Models repository](https://gitlab.com/gitlab-com/gl-security/product-security/appsec/threat-models)(internal link). This also includes templates for threat modeling that anyone can use in issues or epics.
...
A threat model is never really done. The team should be familiar with the threat model and request another App Sec Review or an update to the threat model when new features or changes arise. The Application Security engineer should work closely with the team to also identify when a new threat model is needed.

### Do It Yourself

Much of the language in this guide describes how Application Security engineers should threat model with teams, but that doesn't have to be the case! Security is everyone's responsibility and **you** can threat model too! Reach out to `#security_help` on Slack if you need help.

## Threat Model Agent

### Purpose

The Threat Model Agent is an AI-powered Application Security assistant that helps GitLab team members create, review, and improve threat models by themselves. It applies an Evidence-Driven PASTA framework with STRIDE classification and a 22-point compliance checklist, and supports three workflows:

- Create a new threat model from scratch
- Review and enrich an existing threat model
- Verify an existing threat model for approval
...
The agent is currently only enabled inside the [threat-models repository](https://gitlab.com/gitlab-com/gl-security/product-security/appsec/threat-models). This is an intentional design decision during the initial rollout. Access from other GitLab projects is planned for the future.
...
You can also ask conceptual questions first (e.g., "What does requirement D5b mean?") without committing to a full session. If anything about the process, requirements, or terminology is unclear, ask the agent directly. It can explain individual requirements, severity classification as estimated by the agent, STRIDE categories, or how to scope a feature.

### Creating a Threat Model from Scratch

Tell the agent you want to create a new threat model. It guides you through a structured discovery dialogue, asks for context about the feature, and produces a draft document with a Mermaid Data Flow Diagram, threat table, and data classification. You confirm each threat before it is included.

### Working on an Existing Threat Model

Share the threat model as text, a link, or a file, and tell the agent whether you want a verification or compliance check. The agent flags weak or duplicated threats, suggests missing ones, and returns a coverage score with prioritized improvements.

### Verifying and Reviewing

After drafting or enriching, the agent enters a collaborative refinement loop so you can remove false positives, challenge assumptions, or add context before anything is finalized. All refinements happen in the same conversation.

### Storage Location: Always Verify

The agent automatically decides where the threat model file is stored in the threat-models repository, based on the project's namespace and feature type. The choice is usually correct, but always double-check the file path in the final Draft MR. If the location is wrong, tell the agent where it should go and it will move the file on the same branch.

### Pausing and Resuming with Draft MRs

You can interrupt the process at any point and continue later, if the chat session is still active or a (Draft) MR is created to stash your work on a threat model. MRs are always created in Draft mode and only after you explicitly confirm.

The pause-and-resume workflow:

1. **Pause mid-session**: Stop the conversation whenever needed. Partial answers and confirmed threats are not lost as long as the chat session remains open.
2. **Stash work as a Draft MR**: Once you have enough content to save, ask the agent to create a Draft MR in the threat-models repository. The agent detects the project's default branch, stores the document at the correct namespace-mirrored path, links the originating issue, and appends a session metrics block to the MR description. Verify the chosen path before merging.
3. **Resume later**: Open a new chat with the agent and share the Draft MR link. The agent picks up from the existing document, treating it as input for further verification or refinement.
4. **Iterate on the same branch**: When you make further changes, the agent amends or adds commits to the same branch rather than creating new branches or closing/reopening MRs. This keeps the review history clean.
5. **Finalize**: When the document is complete, assign the MR to the threat model author, remove the Draft status, and request AppSec review.

Draft MRs act as durable storage for in-progress threat models, so you can return to them as the feature design evolves.
