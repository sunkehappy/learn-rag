...
- Post in `#sec-appsec-team-only` with details
...
AppSec's [Triage Rotation](/handbook/security/product-security/security-platforms-architecture/application-security/runbooks/triage-rotation/) handbook page has information on the rotations and who is assigned.

The AppSec team will post a Slack message in `#security_help` with information on who is available, when they are available, and how to get in contact if AppSec assistance is urgently needed.

Each AppSec team member providing coverage will have their mobile phone number available in their Slack profile.

Should an incident occur, the ["Handling S1/P1s procedure"](/handbook/security/product-security/psirt/runbooks/handling-s1p1/) will be followed, which includes handing over to the next AppSec engineer.
