...
GitLab Support handles non-complex application log requests that are within a 1-month time window from the time of the request, and which don't disclose Personal Data, such as user names and IP addresses.
...
1. The Support Engineer or CSM triggers the workflow from Slack with the command `/SIRT`. Select `/security` and the incident responder will direct you to a form to raise a confidential SIRT issue.
...
[Account ownership verification](/handbook/support/workflows/account_verification/) **must** be completed for the account.
