...
Team members working with security vulnerabilities should read this procedure in its entirety and reach out to `@sec-compliance-team` in the `#security-help` Slack channel if you have any questions.
...
##### Software vendor will not release a fix

Software vendor has assessed the vulnerability impact and will not be releasing a security update. These issues have the label `Vulnerability::Vendor Package::Will Not Be Fixed`, and may be closed. The **vulnerability status should be dismissed as "Acceptable risk"**.

##### False positive detections

Once the DR has been opened providing evidence showing a detection is a false positive and does not impact the project, the issue can be closed. The **vulnerability status should be dismissed as "False positive"**.
