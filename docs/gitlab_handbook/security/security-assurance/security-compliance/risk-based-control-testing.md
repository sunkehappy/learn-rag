...
Currently, we provide updates via the epic overview issue and slack at the end of each quarter. We are exploring meaningful metrics to demonstrate results over time.

## Risk-based Control Testing Requests

If you have a process, system, or control that you think should be tested based on the criteria above that we might not be covering already, please follow these steps. Note: to have an entire system (new or changing) assessed, please see the [Security System Intake](./sec-controls.md) process.

### 1. Confirm that the controls are not already tested

Please see the [Program Readme](https://gitlab.com/gitlab-com/gl-security/security-assurance/security-compliance/risk-based-control-testing/-/blob/main/README.md?ref_type=heads) for the schedule to confirm testing is not already planned the process, system, or control.

### 2. Create a Security Compliance Intake Issue

Create a [Recommendation Issue](https://gitlab.com/gitlab-com/gl-security/security-assurance/security-compliance/risk-based-control-testing/-/issues/new?description_template=Risk-based%20Control%20Testing%20Recommendation).

### 3. Security Compliance completes the following workflow

- Confirms current state of testing in GAS
- Reviews the control based on the criteria above and details provided in recommendation issue
- Provides feedback to requester in issue
- Updates the quarterly testing schedule as needed
