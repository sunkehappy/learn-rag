...
**General access** controls who a linked file can be shared to using a link. `Employees only` makes the file linkable only to GitLab permanent team members. `GitLab` makes the file linkable to all internal audiences.
