...
There is not enough capacity to manually triage issues without a group label at present.
We rely on a combination of self and [AI triage](https://gitlab.com/gitlab-org/quality/triage-ops/-/blob/c003f65c94ff55a6b43c0b5c8c8ac0173618c0a5/policies/stages/report/untriaged-issues.yml) to have the type and group labels applied.
...
- Assign a [group label](#group-labels).
  ...
  - Assign a [`~"backlog::prospective label"`](https://gitlab.com/groups/gitlab-org/-/epics/18639) if the issue is planned for resolution within the next 18 months. Assign [`~"backlog::no-commitment"`](https://gitlab.com/groups/gitlab-org/-/epics/18639) otherwise.
...
- Any issue with `~"type::bug"`, `~"Deferred UX"`, or [`~"backlog::triage"`](https://gitlab.com/groups/gitlab-org/-/epics/18639) label has a [priority label](https://docs.gitlab.com/ee/development/labels/index.html#priority-labels) applied.
...
| -------- | ---------- | --------- | --- |
| `~"priority::1"` | Urgent | We will address this as soon as possible regardless of the limit on our team capacity. Our target resolution time is 30 days.                 | PM or EM of that product group, based on work type |
| `~"priority::2"` | High   | We will address this soon and will provide capacity from our team for it in the next few releases. This will likely get resolved in 60-90 days. | PM or EM of that product group, based on work type |
| `~"priority::3"` | Medium | We want to address this but may have other higher priority items. This will likely get resolved in 90-120 days.                               | PM or EM of that product group, based on work type |
| `~"priority::4"` | Low    | We don't have visibility when this will be addressed. No timeline designated.                                          | PM or EM of that product group, based on work type |

## Severity

If you need help estimating severity, reach out in the `#s_developer_experience` channel.

Note: These severity definitions apply to issues only. Please see [Severity Levels section](/handbook/engineering/infrastructure-platforms/incident-management/#severities) of the [Incident Management page](/handbook/engineering/infrastructure-platforms/incident-management/) for details on incident severity.

[Severity labels](https://docs.gitlab.com/ee/development/labels/index.html#severity-labels) help us determine urgency and clearly communicate the impact of a `~"type::bug"`, and `~"type::maintenance"` on users. There can be multiple categories of a type.

For bugs, the presence of bug category labels `~"bug::availability"`, `~"bug::performance"`, `~"bug::vulnerability"`, and `~UX` denotes to use the severity definition in that category. When a `~"type::bug"` correspond to multiple categories, the severity to apply should be the higher, for example, if an issue has a `~"severity::2"` for `~"bug::availability"` and a `~"severity::1"` for `~"bug::performance"` then the severity assigned to the issue should be `~"severity::1"`.
For maintenance issues, the presence of [`~"backlog::triage"`](https://gitlab.com/groups/gitlab-org/-/epics/18639) denotes that severity definition must be specified.
...
|----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| General bugs                                                                           | Broken feature with no workaround or any data-loss.                                                                                                    | Broken feature with an unacceptably complex workaround.                                                                                                | Broken feature with a workaround.                                                                                                                                      | Functionality is inconvenient.                                                                                                                         | [Engineering Manager](/handbook/engineering/careers/management/) for that product group.                                                      |
...
All `~"type::bug"` issues must have one of the following:

1. [`~"backlog::prospective"`](https://gitlab.com/groups/gitlab-org/-/epics/18639) with updates every 6 months, including justification on why it is remaining open past the SLO once applicable.
1. [`~"backlog::no-commitment"](https://gitlab.com/groups/gitlab-org/-/epics/18639)` with justification.
1. justification on why it is being closed without remediation.
...
Issues with `~"bug::availability"` label directly impacts the availability of GitLab SaaS (GitLab.com or a GitLab Dedicated Tenant). It is considered as another category of `~"type::bug"`.

For the purposes of [Incident Management](/handbook/engineering/infrastructure-platforms/incident-management/), incident issue severities are chosen based on the `availability` severity matrix below.

We categorize these issues based on the impact to the customer's business goal and day to day workflow.
...
|-|-|
| `~"severity::1"` | Absence of this limit enables a single user to negatively impact availability of GitLab |
...
[Triage Operations](/handbook/engineering/infrastructure-platforms/developer-experience/triage-operations/)
...
If you are triaging one of these issues, please make your best effort to assign a group label to the issue as soon as possible after creation. You don't have to get it perfect, but just make a conscious effort to identify the group that is the best one set up for success to work on the issue.
