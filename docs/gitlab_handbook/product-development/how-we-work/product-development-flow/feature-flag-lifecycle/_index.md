...
This page defines the process for the feature flags lifecycle at GitLab. Technical information for GitLab developers and code contributors can be found in the [developer documentation](https://docs.gitlab.com/development/feature_flags/).
...
1. New features in high traffic areas (such as, a new merge request widget, a new option in issues/epics, a new CI functionality).
1. Complex security or performance improvements that may require additional testing in production (such as, rewriting complex queries, reusing complex finders in new ways, destructive database updates or changes to frequently used API endpoints).
1. Invasive changes to the user interface (such as, introducing a new navigation bar, removal of a sidebar, significant UI changes in issues or MR interface).
1. Introducing dependencies on third-party services (such as, adding support for importing projects).
1. Changes to features that can cause data corruption or cause data loss (such as, features that process repository data or user-uploaded content).
1. Changes to permissions that will add restrictions (such as, requiring Owner permissions instead of Developer)

Situations where you might consider not using a feature flag:

1. Adding a new API endpoint.
1. Introducing new features in low traffic areas (such as, adding new export functionality in the admin area, group settings area, or project settings area).
1. Non-invasive frontend changes (such as, slightly changing the color of a button, or moving a UI element in a low traffic area).
...
For perspective on why we limit our use of feature flags please watch the video on using [Feature flags only when needed](https://www.youtube.com/watch?v=DQaGqyolOd8).
...
Before using feature flags, make sure to read the following guidance:

1. [Developing with feature flags](https://docs.gitlab.com/ee/development/feature_flags/): Learn about the types of feature flags, their definition and validation, how to create them, frontend and backend details, and other information.
1. [Documenting features deployed behind feature flags](https://docs.gitlab.com/ee/development/documentation/feature_flags.html): How to document features deployed behind feature flags according to their state, and how to update the docs when their state changes.
1. [Controlling feature flags](https://docs.gitlab.com/ee/development/feature_flags/controls.html): Learn the process for deploying a new feature, enabling it on GitLab.com, communicating the change, logging, and cleaning up.
...
1. [How GitLab administrators can enable and disable features behind flags](https://docs.gitlab.com/ee/administration/feature_flags.html): An explanation for GitLab administrators about how they can enable or disable GitLab features behind feature flags.
1. [What "features deployed behind flags" means to the GitLab user](https://docs.gitlab.com/ee/user/feature_flags.html): An explanation for GitLab users regarding how certain features might not be available to them until they are enabled.
