...
## Finding a Subscription through Opportunity

When working on internal request, an opportunity link is provided in the ticket. Sometimes, the opportunity does not match the existing subscriptions found on the customers portal account. The following steps can be used to find the subscription.

1. On the SFDC opportunity page, check the `Related List Quick Links` sections and hover on the `Quotes`.
2. If there are multiple quotes that show up, select the one that's marked as `Sent to Z-Billing`.
3. On the quote page, you can see the `Subscription Name`. You can use it to search for the subscription on Zuora.
   - If you scroll down, you can also see the `Zuora Account ID` and `Zuora Subscription ID`.
