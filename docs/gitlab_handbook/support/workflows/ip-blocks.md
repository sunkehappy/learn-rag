...
Users of GitLab.com can find that their IP address has been blocked due to rate limiting. Currently, rate limit parameters on GitLab.com are best described on the [GitLab.com settings docs page](https://docs.gitlab.com/user/gitlab_com/#account-and-limit-settings). The single source of truth for rate limiting on GitLab.com exists in our [infrastructure rate limiting documentation](/handbook/engineering/infrastructure-platforms/rate-limiting/). This page is intended to help Support Engineers troubleshoot issues related to rate limits and IP blocks.
...
Once you obtain this information, you should [follow our guide](../../engineering/infrastructure-platforms/rate-limiting/troubleshooting.md) for troubleshooting Cloudflare. Some blocks may happen as a result of a mitigation effort, so you may want to verify that a [contact request](https://gitlab.com/gitlab-com/support/internal-requests/-/issues) is not open on the internal board.
...
## Unable to find any log entries in Cloudflare or Kibana?

In cases where you are unable to find any log entries in Cloudflare or Kibana for an IP block, it may be required to reach out to our Production Engineering team. We can [request additional help from the Production Engineering team](/handbook/engineering/infrastructure-platforms/rate-limiting/troubleshooting/#requesting-further-assistance) in scenarios like this.

## False positive IP geolocation block

In the case where a user is being blocked from accessing GitLab.com and it is suspected to be a false positive geolocation block we can [request additional help from the Production Engineering team](/handbook/engineering/infrastructure-platforms/rate-limiting/troubleshooting/#requesting-further-assistance).
...
Please refer to our [Bypass Policy](../../engineering/infrastructure-platforms/rate-limiting/bypass-policy.md) for more information.
