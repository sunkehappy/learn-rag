...
fit our [definitions of support impact for a severity 1 case](https://support.gitlab.com/hc/en-us/articles/11626416629660-Definitions#severity-1--urgent). This workflow
...
#### GitLab Geo secondary site is not replicating

- **Emergency**:  The customer has a disaster recovery failover exercise planned for tomorrow in order to meet regulatory requirements.
- **Not Emergency**: The customer has a disaster recovery failover exercise planned for next month in order to meet regulatory requirements.
- **Reasoning**:  Not executing a planned disaster recovery failover exercise will create immediate compliance problems for a customer's business and may even result in fines. If timelines are very short, this qualifies for an emergency. Otherwise, with customer agreement this situation may be treated as a normal support ticket with priority set to 'high' as it is still important to restore the secondary site quickly for business continuity purposes.

#### GitLab Geo secondary site is unhealthy, but replicating slowly

- **Emergency**: See the ["not replicating" case](#gitlab-geo-secondary-site-is-not-replicating), or if replication lag is not decreasing at a reasonable rate.
- **Not Emergency**: [Replication is lagging](https://docs.gitlab.com/administration/geo/replication/troubleshooting/common/#check-the-health-of-the-geo-sites) but progressing at a reasonable rate towards a healthy state. For example, after a large amount of data was added to the Primary, and the Secondary's networking bandwidth can't be increased.
- **Reasoning**: Replication _performance_ can be low for many different reasons, some of which are [out of scope reasons](https://about.gitlab.com/support/statement-of-support/#configuration-of-deployment-environments). The duration of an investigation should be in a sensible relation to the amount of speed-up that can be expected.
...
- **Reasoning**: Security incidents require an immediate response from Support to prevent or limit unauthorized access and potential data compromise.

#### Security incident affecting GitLab.com

- **Emergency**: Passwords or tokens have been leaked or exposed, potentially allowing unauthorized access to the customer's GitLab.com resources. Follow the [GitLab.com SIRT emergency workflow](customer_emergencies_workflows.md#gitlabcom-sirt-emergency) to handle this type of emergency.
- **Reasoning**: Security incidents require an immediate response from Support to prevent or limit unauthorized access and potential data compromise.

#### GitLab Duo Pro/Enterprise features are unavailable

- **Emergency**: GitLab Duo Pro/Enterprise features are completely unavailable or severely degraded for all users
- **Not Emergency**: GitLab Duo Core issues, or GitLab Duo Pro/Enterprise issues have minimal business impact or a viable workaround that the customer can implement immediately to unblock
- **Reasoning**: Customers pay specifically for GitLab Duo Pro/Enterprise as add-on services, so unavailability directly impacts their paid investment and service expectations
