...
- Removing an identity tied to the user account. The identity should also be verified in these cases.
- Adding a secondary email address to a paid user's account, such as when security emails cannot be delivered for any reason.
...
The primary and secondary email can be released for paid users only, following one of the processes below.

### Release an unverified secondary email address

1. Confirm the email address as an unverified secondary email address.
1. If the ticket requester email does not match, perform an email verification by asking the customer to comment on the ticket from the secondary email.
...
1. Reply to the user saying that their email address is now released and they can use it to create a new account.
...
1. Once the user has replied to confirm they own the email address, update the email address with `+release`. For example, if the email address is `johndoe@example.com`, then update the email address on the account to `johndoe+release@example.com`.
    ...
    - If release is done through the UI, the original email needs to be deleted from the list of secondary emails.
      Chatops will handle this automatically.
1. Add an [Admin Note](/handbook/support/workflows/admin_note/) to the user's account.
...
1. Check whether the new email domain is verified by another group using the [Pages Domain API](https://docs.gitlab.com/api/pages_domains/#list-all-pages-domains) from an admin account. If it is, add an **internal note** on the ticket.
1. Ask manager approval to proceed and respond to the customer:
...
1. Check whether the new email domain is verified by another group using the [Pages Domain API](https://docs.gitlab.com/api/pages_domains/#list-all-pages-domains) from an admin account. If it is, add an **internal note** on the ticket.
1. Send a first response to the user:
