...
**NOTE:** For internal requests relating to licenses and subscriptions, please refer to the [relevant license and subscription workflow](/handbook/support/license-and-renewals/workflows/working_internal_requests/), or [CustomersDot console workflow](/handbook/support/license-and-renewals/workflows/customersdot/customer_console/).
...
**Use Project Admin Import for**

- Customer is stuck with project import
- The delay has business impact for the customer
- You have already created a RFH to Import team, who approved the action, and confirmed they cannot fix the issue for the customer in a timely manner,

**GitLab.com Admin Escalation for**

- For bulk or single update actions that can be done via API
- The action is approved by manager
- The scripts/commands to be run were approved by relevant product or security team

### Account ownership verification

[Account Ownership Verification](/handbook/support/workflows/account_verification) is required for both templates. If the action would be affecting a number of group members, they need to match the definition of enterprise users.

## GitLab.com Console Escalation

Templates available for requesting an engineer with GitLab.com console access to take action after confirming it cannot be easily done via API:

1. [Console Export Request](https://gitlab.com/gitlab-com/support/internal-requests/-/issues/new?description_template=GitLab.com%20Console%20Export%20Request) - For creating a project export.
1. [Console Escalation (Read-only)](https://gitlab.com/gitlab-com/support/internal-requests/-/issues/new?description_template=GitLab.com%20Console%20Escalation%20(Read-only)) - For safe operations like investigations and data gathering.
1. [GitLab.com Console Escalation (Read/write)](https://gitlab.com/gitlab-com/support/internal-requests/-/issues/new?description_template=GitLab.com%20Console%20Escalation%20(Read-write)) - For operations that modify production data.

### Choosing the right template

**Use Read-only template for:**

- Investigating potential bugs or understanding object states
- Gathering large datasets where the API is insufficient
- Any operation that doesn't modify production data

**Use Read/write template for:**

- Operations that modify production data
- Failed project, group, or user deletions
- Running one-off scripts or commands
- Any operation with potential risk to production
...
### Console escalation best practice and general guidelines

The rails console is typically used when we are unable to retrieve information or fix an issue via the UI or API. Common issues include the following when the UI and API methods are not available:

- Deletion of projects, groups, container registry images, etc. where it failed
- User account issues
- Data inconsistencies and understanding the state of data
- Bulk actions that are not available in the UI/API
- Debugging issues that cannot be replicated

Rarely, console escalations can be used to work around missing features where Support can complete actions that would be unavailable or onerously time-consuming to customers. When actions cannot be done via the UI or API, engineers should ensure that a feature request exists to have it in the product. Consider tagging the appropriate Product Manager for prioritization.

We should always prioritize bug fixes and product improvements over manual and potentially risky console interventions.
...
For any read-write requests, the issue requires:

- Review by a subject matter expert or Request for Help (RFH) issue
- Developer/PM/EM backing for one-off fixes with clear justification
- Account ownership verification for customer-requested actions

Any request requiring assistance from infrastructure teams, please see the [SaaS Platforms Request for Help](https://gitlab.com/gitlab-com/saas-platforms/saas-platforms-request-for-help) issue tracker and [SaaS Platforms Getting Assistance](../../engineering/infrastructure-platforms/getting-assistance/) page.
