---
title: How to be a Support Leader on the Hook (SLOTH 🦥)
category: On-call
description: "Describes the role and responsibilities for the Support Leader on the Hook (SLOTH 🦥) rotation in Support Engineering"
---

## Introduction

The Support Leader On the Hook (SLOTH 🦥) helps coordinate responses to urgent and important situations that arise within the scope of delivering a quality
experience to GitLab customers.

SLOTH is one of the rotations that make up [GitLab Support On-call](/handbook/support/on-call/).

## Expectations for the SLOTH role

The SLOTH role is a follow-the-sun 24/7 rotation staffed in-seat on regular workdays
and on-call on weekends, public holidays and other forms of time off.

SLOTH is responsible generally for:
...
The [Support Engineer on-call](/handbook/support/on-call/) is the first responder for customer emergencies. SLOTHs support this work as follows:
...
- During an emergency: find additional staff for subject-matter expertise; replace the on-call engineer if needed; lead any Zoom call(s) as needed; hand off the emergency to the next SLOTH.
- Find additional staff when there are [multiple emergencies](/handbook/support/workflows/customer_emergencies_workflows#handling-multiple-simultaneous-emergencies).
...
At times, an emergency page may come in for a situation that is not quite yet an emergency, but may quickly become one. In this situation, we want to assist the customer in preventing the situation from becoming an emergency. If this situation arises during working hours, the Support Engineer on-call may reach out for assistance. The SLOTH should respond by finding additional staff to handle the request as a `high` priority ticket that requires an immediate response. If this situation arises on a weekend, the Support Engineer on-call will reach out to the SLOTH if they are handling another emergency. In that case, the SLOTH should assist or attempt to find additional staff to assist.
...
If you are SLOTH and a concurrent emergency occurs, you will be paged by the Support Engineer On-call escalated via Pagerduty. You will then be responsible for checking the current situation and determining if the **backup engineers** need to be paged. If so, SLOTH will then **manually page** the **backup engineers**. At this point, the backup engineers are all pinged. Only one backup engineer needs to acknowledge the page and lend assistance, and there is no expectation that **backup engineers** will be available to respond to a page.
...
STARs ([Support Ticket Attention Requests](/handbook/support/internal-support/support-ticket-attention-requests)) are handled by SLOTH.
...
1. When someone initiates a STAR using the [STAR Form](https://gitlab-com.gitlab.io/support/toolbox/forms_processor/support_escalation):
   ...
   1. **BOT**: Slack announcement is posted to `#support_ticket-attention-requests`, with an `@mention` of the current on-call SLOTH's name.
...
In case an Account Escalation opens where support involvement is needed, it is the responsibility of the SLOTH to find a Support Manager DRI. In case this is an ASE account (you can find this in the org notes) please ping Lissa Roberts (AMER), Ilia Kosenko (EMEA) or Wei Meng Lee (APAC).
...
[Special handling notes](/handbook/support/workflows/customer_emergencies_workflows#special-handling-notes) are documented on the [customer emergencies on-call workflow](/handbook/support/workflows/customer_emergencies_workflows). As a SLOTH, you are empowered to handle these (and other) unique situations
...
We advise Support Engineers to contact a SLOTH before offering a call in the case of a [compromised instance](/handbook/support/workflows/customer_emergencies_workflows#sirt-emergencies).
