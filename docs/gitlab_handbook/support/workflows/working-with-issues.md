...
In general, the Product team  will [prioritize all issues](/handbook/product/product-processes/cross-functional-prioritization/) (not just customer requests) based on types of issues and the [direction of the product](https://about.gitlab.com/direction/).
...
- Advanced search tips:
  ...
  - See other available syntax options from the [Advanced Search Docs](https://docs.gitlab.com/user/search/advanced_search/#syntax)
...
- You are likely to spend some time searching for issues you've read before.
  ...
  - For quickly finding things based on your own comments, consider setting up the [User Contribution Search](https://gitlab.com/gitlab-com/cs-tools/gitlab-cs-tools/user-contribution-search) tool. It indexes all your contributions and gives you a full text search interface.
...
- `~"Customer Interest` (if a ZenDesk link is added, the bot will add this automatically if you forget)
...
For `~"Customer Interest` and any label that allows severity (most commonly `~"type::bug"`) labeled issues, a Severity estimate is required. If it is missing, please add them to attract PM attention to the issue:
...
For L&R related issues, please see the [specific guidance on Fulfillment issues](/handbook/support/license-and-renewals/workflows/managing_product_issues/).
...
| Support Engineers with GitLab.com console access| Anything related to the **use of GitLab.com**, operations that can't be performed with admin access  | See [Console escalation requests](/handbook/support/workflows/internal_requests#gitlabcom-console-escalation) | Use `~"GitLab.com Console Escalation"` label |
