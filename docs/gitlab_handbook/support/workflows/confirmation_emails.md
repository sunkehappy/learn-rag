...
the [SaaS Account Ticket Helper](/handbook/security/customer-support-operations/zendesk/apps/global#gitlab-super-app) application can activate.
...
As of 2023-08-07, unconfirmed users are [deleted after a set number of days](https://docs.gitlab.com/user/gitlab_com/#confirmation-settings) on GitLab.com.
...
1. Access the log details by clicking on the ellipsis `...` icon at the right end of the log entry.
...
1. On the ellipsis `...` icon at the right end of the log entry, click on the "Resend message" button.
...
1. Click on `Send` along the left-hand side navigation bar.
1. Click on `Logs`.
1. Ensure that `mg.gitlab.com` is set as the domain above the activity graph.
